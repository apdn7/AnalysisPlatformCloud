class ParallelPlot {
    ...
    genPCatDimension(colData) {
        ...
        if (isNumber) {
            dim.ticktext = dim.ticktext.map((i) => {
                ...
                return `${applySignificantDigit(i, undefined, colData.col_detail.format)}`;
            });
        } else {
            dim.ticktext = dim.ticktext.map((i) => String(i));
        }
    ...
    orderDimension(dimension) {
        ...
        const otherDimensions = dimension.filter(
            (dim) =>
                dim.dimID !== this.objective.name &&
        ...
        if (this.settings.orderBy === ParallelProps.orderBy.correlation) {
            nonObjectiveDimension = nonObjectiveDimension.sort(
