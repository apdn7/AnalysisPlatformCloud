...
const catTypes = [
    DataTypes.TEXT.name,
    DataTypes.TEXT.bs_value,
    DataTypes.STRING.bs_value,
    DataTypes.CATEGORY.bs_value,
    CONST.CATEGORY,
];
// const catTypes = DataTypes.TEXT.bs_value || DataTypes.STRING.bs_value || CONST.CATEGORY;
const intTypes = [DataTypes.INTEGER.name, DataTypes.INTEGER.bs_value];
// const intTypes = DataTypes.INTEGER.bs_value || DataTypes.INTEGER.name;
...
const handleSelectMenuItem = (e, updatePosition = false) => {
    ...
    if (selectedDim) {
        const [, dim] = selectedDim.split('-').map((i) => i);
