const generateViolinPlot = (prop, zoomRange, option) => {
    ...
    const isXIntType =
        ...
        (option.xDataType === DataTypes.STRING.name &&
            !Number.isNaN(Number(prop.array_x[0])));
    const isYIntType =
    ...
    const layout = {
        ...
        yaxis: {
            ...
            tickformat: option.yFmt.includes('e') ? '.1e' : option.yFmt,
        },
        xaxis: {
            showline: true,
            showgrid: true,
            ...
            tickformat: option.xFmt.includes('e') ? '.1e' : option.xFmt,
...
const makeHoverInfoBox = (prop, key, option, x, y, is_horizontal = false) => {
    ...
    let x_index, y_index;
    if (is_horizontal) {
        y_index = prop.array_y.indexOf(key);
    } else {
        x_index = prop.array_x.indexOf(key);
    }
    const [x_master, y_master] = getXYMasterInfo(prop, x_index, y_index);
