...
const genGraph1DMatrix = (inputGraphs, asc = false) => {
    ...
    for (let i = 0; i < graphs.length; i++) {
        if (Number(graphs[i].h_label)) {
            graphs[i].h_label = applySignificantDigit(
                Number(graphs[i].h_label),
                undefined,
                graphs[i].h_label_fmt,
            );
        }
...
const gen2DMatrixTitle = (graphs) => {
    ...
    for (const graph of graphs) {
        if (!cols.includes(graph.h_label)) {
            if (Number(graph.h_label)) {
                graph.h_label = applySignificantDigit(
                    Number(graph.h_label),
                    undefined,
                    graph.h_label_fmt,
                );
            }
...
const genScatterPlots = (
    ...
    const chartOptions = {
        ...
        xFmt: res.x_fmt || '',
        yFmt: res.y_fmt || '',
        xUserFmt: res.x_user_fmt || '',
        yUserFmt: res.y_user_fmt || '',
...
const genLinearColorBar = (colorRange, isIntColor = false, fmt) => {
    ...
    const ticks = tickValues.map((tick) => {
        const tickValue = applySignificantDigit(
            colorRange[0] + tick * stepValues,
            4,
            newFmt,
        );
        if (isIntColor) {
...
const makeScatterHoverInfoBox = (prop, option, key, pageX, pageY) => {
    ...
    let hTitle = prop.data.customdata.htitle;
    const arxValue = prop.data.customdata.array_x;
    const arxFormat = prop.data.customdata.columndata.x_fmt;
    const aryValue = prop.data.customdata.array_y;
    const aryFormat = prop.data.customdata.columndata.y_fmt;
    ...
    const colFormat = prop.data.customdata.columndata.color_fmt;
    ...
    const getPointTime = (pointer) =>
        pointer
            ...
            : '';
    const getPointValue = (i, x_fmt, y_fmt) => {
        const xValue = applySignificantDigit(arxValue[i], undefined, x_fmt);
        const yValue = applySignificantDigit(aryValue[i], undefined, y_fmt);
        return {
            x: xValue,
            y: yValue,
        };
    ...
    const colorVal = orgColorValue[key]
        ? applySignificantDigit(orgColorValue[key], undefined, colFormat)
        : '';
    const pointValue = getPointValue(key, arxFormat, aryFormat);
...
const genVLabels = (
    ...
    vLabels.forEach((vLabel, i) => {
        vLabel = formatCatExpBox(vLabel, facetFormat);
...
const showSCP = async (
    ...
    if (res) {
        ...
        consoleLogDebug(
            `[ScP][showSCP]\nx_id: ${res.x_id}\nx_is_master: ${res.x_is_master}\nx_label: ${res.x_label}\ny_id: ${res.y_id}\ny_is_master: ${res.y_is_master}\ny_label: ${res.y_label}\n`,
        );
        ...
        const showGraph = (settings = { isShowForward: false }) => {
            const scpCloneData = jsonParse(JSON.stringify(scpData));
            ...
            if (!autoUpdate) {
                // scroll to chart
                const scpCardPosition = $('#colorSettingGrp').offset().top;
...
const getXYMasterInfo = (prop = {}, x_index = 0, y_index = 0) => {
    // ↓--- Get Master Info for hover ---↓
    let x_master = [];
    let y_master = [];
    for (const i of ['x', 'y']) {
        let details;
        ...
        if (prop[`${i}_is_master`] === true) {
            const masterName = prop[`${i}_name`];
            for (const master_id of prop[`array_${i}_master`][index]) {
                const detail = getMasterInfoHovering(master_id, masterName);
                details.push(detail);
            }
        }
    }
