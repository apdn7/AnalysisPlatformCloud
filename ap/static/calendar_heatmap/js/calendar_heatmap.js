...
const drawHeatMapFromPlotData = (canvasId, plotData) => {
    ...
    const colorSelectDOM = numericDataTypes.includes(plotData.data_type)
        ? eles.colorReal
        : eles.colorCat;
...
const drawHeatMap = (orgData, scaleOption = 'auto', autoUpdate = false) => {
    ...
    const buildGraphTitle = (plotData, procId, facetFormats) => {
        ...
        if (!isEmpty(cateValue)) {
            if (Array.isArray(cateValue)) {
                facetLabel = formatCatExpBox(cateValue.join('|'), facetFormats);
            } else {
                facetLabel = formatCatExpBox(cateValue, facetFormats);
            }
            title = `${end_proc_name}-${sensorName}`;
        }
        return [title, sensorName, cateValue, facetLabel, isCTCol];
    };
    ...
    for (let rowIdx = 0; rowIdx < orgData.row; rowIdx++) {
        ...
        for (const plotIdx in arrayPlotData) {
            ...
            const [title, sensorName, cardValue, facet, isCTCol] =
                buildGraphTitle(plotData, procId, data.COMMON.catExpBoxFormat);
