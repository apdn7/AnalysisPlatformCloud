...
const getAllDatabaseConfigTbView = async () => {
    const json = await fetchWithLog('/ap/api/setting/database_tables_source', {
        method: 'GET',
        headers: {
...
const getColumnNames = async (database, tableName) => {
    ...
    const json = await fetchWithLog(`${url}?${paramString.toString()}`).then(
...
const showSortColumnOptions = async (procId) => {
    ...
    const tableName = procCfg.data_tables[0].table_name
        ? procCfg.data_tables[0].table_name
        : '';
...
const queryRecordsFromDB = (
    ...
    fetchWithLog('api/table_viewer/table_records', {
        method: 'POST',
        headers: {
            Accept: 'application/json',
            'Content-Type': 'application/json',
