...
const multipleScatterPlot = (data, clearOnFlyFilter = true) => {
    ...
    $('.hist-item div').each((k, his) => {
        ...
        const [histTrace, fmt, is_user_fmt] = genHistogramTrace(
            plotData,
            xScaleOption,
        );
    ...
    $('.sctr-item div.center').each((k, sct) => {
        ...
        let { x_fmt, y_fmt, x_user_fmt, y_user_fmt } = scatterData;
        const [is_x_user_format, is_y_user_format] = [
            Boolean(x_user_fmt),
            Boolean(y_user_fmt),
        ];
        [x_fmt, y_fmt] = [
            is_x_user_format ? x_user_fmt : x_fmt,
            is_y_user_format ? y_user_fmt : y_fmt,
        ];
        ...
        sctPlot.on('plotly_hover', (data) => {
            ...
            const xValue = applySignificantDigit(
                dataPoint.data.x[dpIndex],
                undefined,
                x_fmt,
            );
            const yValue = applySignificantDigit(
