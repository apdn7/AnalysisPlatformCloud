...
const showTabsAndCharts = (
    ...
    const catExpFormat = data.COMMON.catExpBoxFormat;

    for (let sensorIdx = 0; sensorIdx < numSensors; sensorIdx++) {
        if (onlySensorId !== null && onlySensorId !== sensors[sensorIdx]) {
            continue;
        }
        ...
        const sensorPlotDatas =
            eleIdPrefix !== 'directTerm'
                ...
                : data.array_plotdata.filter((plot) => plot.end_col === sensor);
        if (!sensorPlotDatas) {
            continue;
        }
        ...
        // カラム名を取得する。
        const {
            end_proc_name: endProcName,
            end_proc_id: endProcId,
            end_col: endColId,
        } = sensorPlotDatas[0];
        const displayColName = getColumnName(endProcId, endColId);

        const isCategory = sensorPlotDatas[0]
            ? sensorPlotDatas[0].is_category
            : false;
        const fmt = procConfigs[endProcId].dicColumns[sensor].format ?? '';
        const sensorType =
            procConfigs[endProcId].dicColumns[endColId].data_type;
        const allGroupNames =
            isCategory ||
        ...
        for (let i = 0; i < numGraphs; i++) {
            ...
            const catExpValues =
                catExpBoxCols.length > 0
                    ...
                    : [];
            const catExpValuesFormatted = catExpValues.map((v, index) =>
                applySignificantDigit(
                    parseInt(v) || v,
                    undefined,
                    catExpFormat[index],
                ),
            );
            const catExpFormatted = catExpValuesFormatted.join(' | ');
            const filterCond =
                catExpValuesFormatted.length > 0 ? catExpValuesFormatted : null;
            ...
            const end_col_format = sensorPlotDatas[i].end_col_format;
            const format = end_col_format ? end_col_format : '';
            generalInfo['endProcFmt'] = format;
            const columnName = sensorPlotDatas[i].end_col_name;
            const columnId = sensorPlotDatas[i].end_col;
