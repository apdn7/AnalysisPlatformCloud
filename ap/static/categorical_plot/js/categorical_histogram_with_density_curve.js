const HistogramWithDensityCurve = ($, paramObj) => {
    ...
    const endColFmt = setParam('endColFmt', '');
    const rankColsToMasterIds = setParam('rankColsToMasterIds', []);
    const columnName = setParam('columnName', '');
    ...
    const histogram = {
        ...
        hoverinfo: 'none',
        // hovertemplate: '(%{y}, %{x})',
        yhoverformat: endColFmt ? endColFmt : labelFmt,
    };
    ...
    // format group name values
    if (allGroupNames.value && _.isArray(allGroupNames.value)) {
        allGroupNames.value = allGroupNames.value.map((val) => {
            if ($.isNumeric(val)) {
                return applySignificantDigit(Number(val), undefined, endColFmt);
            } else {
                return val;
            }
        });
