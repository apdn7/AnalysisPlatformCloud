...
const renderAgPAllChart = (
    ...
    plots.forEach((plotData, i) => {
        ...
        const { end_proc_id, end_col_id, cat_exp_format } = plotData;
        const catExpBox = plotData.catExpBox ? plotData.catExpBox : [];
        const catExpFormat = cat_exp_format || [];

        const catExpBoxAfterFormatted = catExpBox.map((v, index) =>
            applySignificantDigit(v, undefined, catExpFormat[index]),
        );
        const facetLevel1 =
            catExpBoxAfterFormatted.length > 0
                ? catExpBoxAfterFormatted[0]
                : undefined;
        const facetLevel2 =
            catExpBoxAfterFormatted.length > 1
                ? catExpBoxAfterFormatted[1]
                : undefined;
