...
const updateRecordInfo = (dataInfos = {}, sampleNo = 0) => {
    ...
    for (const dataInfo of dataInfos) {
        let [proc, col_name, col_val, col_attr, col_fmt] = dataInfo;
        let bgColorStyle = '';
        if (re.test(col_attr)) {
        ...
        let showVal = col_val && !_.isObject(col_val) ? col_val : '';
        if (col_fmt) {
            showVal = applySignificantDigit(showVal, undefined, col_fmt);
        } else if (bgColorStyle == '') {
            showVal = applySignificantDigit(col_val);
        }
        content += `
            < tr >
                <td ${bgColorStyle}>${proc}</td>
                <td ${bgColorStyle}>${col_name}</td>
                <td ${bgColorStyle}>${showVal}</td>
            </tr > `;
    }
