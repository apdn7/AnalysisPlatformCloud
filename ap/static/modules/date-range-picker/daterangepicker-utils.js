/**
 * @file Manages the configuration settings for date time range picker control.
    ...
    const initDateTimePickerInner = (datetimeRangeControls) => {
        datetimeRangeControls.each((i, element) => {
            ...
            if (isShowRecentDates) {
                _recentChosenDateRanges = jsonParse(localStorage.getItem(RECENT_DATE_RANGES_LS_KEY));
                ...
                for (let i = 0; i < _recentChosenDateRanges[key].length; i++) {
                    ...
                    const objStartDate = moment(strStartDate, config['locale']['format']);
                    const objEndDate = moment(strEndDate, config['locale']['format']);
            ...
            if (!isSetDefaultValue) {
                $element.on('apply.daterangepicker', function(ev, picker) {
                    $(this).val(picker.startDate.format(config['locale']['format']) + DATETIME_PICKER_SEPARATOR + picker.endDate.format(config['locale']['format']));
                    $(this).trigger('change');

                    /** @description ONLY USE FOR MAPPING CONFIG PAGES */
                    if (this.stackEvent) {
                        executeStackEvent(this, 'change');
                    }
                });
...
const initializeDateTimePicker = (dtId = null, isClassName = false, parentEl = null) => {
    ...
    const initDateTimePickerInner = (datetimeRangeControls) => {
        datetimeRangeControls.each((i, element) => {
            ...
            if (isShowRecentDates) {
                _recentChosenDates = jsonParse(localStorage.getItem(RECENT_DATES_LS_KEY));
                ...
                for (let i = 0; i < _recentChosenDates[key].length; i++) {
                    ...
                    const objDate = moment(strDate, config['locale']['format']);
            ...
            // add event for input control
            if (element.stackEvent) {
                // serve in Mapping Config pages
                element.stackEvent.keyup.unshift(updateDateTimeValue);
                element.stackEvent.change.unshift(updateDateTimeValue);
            } else {
                $element.keyup(delay(updateDateTimeValue, KEYUP_EVENT_DELAY));
                $element.change(element => updateDateTimeValue(element));
            }
