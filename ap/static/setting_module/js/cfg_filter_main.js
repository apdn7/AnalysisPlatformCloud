...
const getProcessConfigFromDB = async (procId) => {
    if (!procId) return {};
    const json = await fetchWithLog(
        `/ap/api/setting/proc_filter_config/${procId}`,
        {
            method: 'GET',
            headers: {
...
const loadFilterOthers = async (processConfig) => {
    ...
    if (filters) {
        await filters.forEach(async (filterConfig) => {
            if (
                filterConfig &&
                filterConfig.filter_type === filterTypes.OTHER
            ) {
...
const showProcessSettings = async (procId) => {
    ...
    if (!isEmpty(processConfig)) {
        // clear processConfig
        let processConfigLine = $.extend({}, processConfig);
        let processConfigMachine = $.extend({}, processConfig);
        let processConfigPart = $.extend({}, processConfig);

        for (let index in processConfig.columns) {
            if (
                processConfig.columns[index].column_type ===
                masterDataGroup.LINE_ID
            ) {
                processConfigLine.columns = processConfig.columns[index];
                delete processConfig.columns[index];
            } else if (
                processConfig.columns[index].column_type ===
                masterDataGroup.EQUIP_ID
            ) {
                processConfigMachine.columns = processConfig.columns[index];
                delete processConfig.columns[index];
            } else if (
                processConfig.columns[index].column_type ===
                masterDataGroup.PART_NO
            ) {
                processConfigPart.columns = processConfig.columns[index];
                delete processConfig.columns[index];
            }
        }
