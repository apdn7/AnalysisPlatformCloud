...
const getConfigOption = () =>
    jsonParse(localStorage.getItem('network-config')) || {};
...
const validateMasterColumn = (
    ...
    let isValid = true;
    let message = '';
    selfColumnTypes.forEach(function (selfColumnType, i) {
        if (
            representColumnType.includes(Number(selfColumnType)) &&
            selfColumnType !== targetColumnTypes[i]
        ) {
            isValid = false;
            message = $('#i18nInvalidMasterColumn').text();
            return true;
        }
        if (representColumnType.includes(Number(selfColumnType))) {
            isValid = false;
            message = $('#i18nInvalidSubStrMasterColumn').text();
        }
    });
    return {
        is_valid: isValid,
...
const reOrderLinkCols = (linkingCols) => {
    ...
    for (const linkingCol of linkingCols) {
        ...
        } else if (linkingCol.raw_data_type === DataTypes.DATETIME.bs_value) {
            dateTimeCols.push(linkingCol);
        } else if (
            linkingCol.raw_data_type === DataTypes.DATE.bs_value &&
            linkingCol.column_name === masterDataGroup.MAIN_DATE
        ) {
            mainDateCols.push(linkingCol);
        } else if (linkingCol.raw_data_type === DataTypes.DATE.bs_value) {
            dateCols.push(linkingCol);
        } else if (
            linkingCol.raw_data_type === DataTypes.TIME.bs_value &&
            linkingCol.column_name === masterDataGroup.MAIN_TIME
        ) {
            mainTimeCols.push(linkingCol);
        } else if (linkingCol.raw_data_type === DataTypes.TIME.bs_value) {
            timeCols.push(linkingCol);
        } else if (masterInnerOrder.includes(Number(linkingCol.column_type))) {
            masterCols.push(linkingCol);
        } else if (linkingCol.raw_data_type === DataTypes.CATEGORY.bs_value) {
            catCols.push(linkingCol);
        } else if (linkingCol.raw_data_type === DataTypes.STRING.bs_value) {
            stringCols.push(linkingCol);
        } else if (
            CfgProcess_CONST.INTEGER_TYPES.includes(linkingCol.raw_data_type)
        ) {
...
const getInforToGenerateColumns = (selfProcId, targetProcId, edgeData) => {
    ...
    selfColumnTypes = selfSerialColumns.map((e) => e.column_type);
    selfColIsMasterCol = selfSerialColumns.map((e) => e.is_master_col);
    ...
    targetColumnTypes = targetSerialColumns.map((e) => e.column_type);
    targetColIsMasterCol = targetSerialColumns.map((e) => e.is_master_col);
    ...
    checkedVals.forEach((selector, i) => {
        ...
        let options = itemAliases
            .map(
                (v, k) => {
                    is_show_digit_match = !(
                        `${v}` === `${selector}` && isMasters[k]
                    );
                    const selected =
                        `${v}` === `${selector}` ? ' selected="selected"' : '';
                    return `<option value="${v}"${selected} alias="${itemAliases[k]}" data-type="${dataTypes[k]}" column-type="${columnTypes[k]}"
...
const addTraceKey = (isNew = false) => {
    ...
    const fwdColumns = addGroupListSelection(
        ...
        (isMasters = targetColIsMasterCol),
...
const saveTraceConfigs = (edgesCfg) => {
    fetchWithLog('api/setting/trace_config', {
        method: 'POST',
        headers: {
...
const updateProcessEditModal = (procs = []) => {
    ...
    if (Array.isArray(procs)) {
        procs.map((proc) => {
            ...
            // brige7 use name and shown name for process without generate English name
            // TODO(merge-code-208) bridge7 use english name now ...
            if (proc.name_en) {
...
const updateNodeInfo = (
    ...
    for (const procId in predictionNodes) {
        ...
        consoleLogDebug(
            `[updateNodeInfo]\n${JSON.stringify(nodes.get(procId))}`,
        );
    }
};
    ...
    for (const edge in predictionEdges) {
        ...
        for (const edgeId of edges.getIds()) {
            ...
            if (`${from}` === `${procStart}` && `${to}` === `${procEnd}`) {
                ...
                consoleLogDebug(
                    `[updateEdgeInfo]\n${JSON.stringify(edges.get(edgeId))}`,
                );
            }
        }
    }
...
const getEdgeFromUI = () => {
    ...
    edgeData.target_orig_col = targetOrgCols;
    edgeData.target_substr = targetSubStrs;
    edgeData.target_column_type = targetColumnTypes;
    ...
    edgeData.self_column_type = selfColumnTypes;
    return edgeData;
};
...
const saveEditEdge = () => {
    ...
    const validMasterCol = validateMasterColumn(
        edgeData.self_column_type,
        edgeData.target_column_type,
        edgeData.self_substr,
        edgeData.target_substr,
    );
    if (!validMasterCol.is_valid) {
        displayRegisterMessage('#alertMsgCheckSubStr', {
            message: validMasterCol.message,
            is_error: true,
        });
        return;
    }
...
const updateTraceConfig = (element, index) => {
    ...
    const columnType = Number(optionSelected.attr('column-type'));
    ...
    if (dataType === DataTypes.DATETIME.bs_value) {
    ...
    if (dataType !== DataTypes.DATETIME.bs_value) {
        ...
        targetColParent
            ...
            .remove();
        // not show link by substring for master column
        if (masterInnerOrder.includes(columnType)) {
            targetColParent.find(tracingElements.edgeConfigSerial).hide();
        } else {
