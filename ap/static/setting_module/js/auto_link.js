...
const collectCheckedProcessesForAutoLink = () => {
            ...
            if (datetimeCols.length && serialCols.length) {
                if (dbsType === DB_CONFIGS.CSV.configs.type) {
                    ...
                    selectedProcesses.push({
                        data_source_id: proc.data_sources[0].id,
                        path: proc.data_sources[0].csv_detail.directory,
                        processes: [proc.name],
                        ids: [proc.id],
                        date_col: datetimeCols[0].column_name,
                        serial_col: serialCols[0].column_name,
                        dbtype: dbsType,
                        delimiter: proc.data_sources[0].csv_detail.delimiter,
                    });
                } else {
                    // Factory DB
                    selectedProcesses.push({
    ...
    for (const path of Object.keys(selectedV2Processes)) {
        const v2ProcNames = selectedV2Processes[path].map(
            (proc) => proc.data_sources[0].csv_detail.process_name,
        );
    ...
    for (const path of Object.keys(processObj)) {
            ...
            for (const i in procs) {
                ...
                child += `
                   <li class="custom-control custom-checkbox">
                        <input type="checkbox" dbs-type="${proc.data_sources[0].type}" dbs-id="${proc.data_sources[0].id}" data-path="${path}" id="${path}-${proc.name_en}" ${proc.isChecked ? 'checked' : ''} name="process" class="custom-control-input" value="${proc.id}">
        ...
        groupHtml += `
               <li class="custom-control custom-checkbox ${hasChild ? 'has' : ''}">
                   <span class="tree-plus expanded"></span>
                   <input type="checkbox" dbs-type="${procs[0].data_sources[0].type}"
                        dbs-id="${procs[0].data_sources[0].id}" data-path="${path}"
                        db-type="${procs[0].data_sources[0].type}" name="${hasChild ? 'dbSource' : 'process'}"
...
const groupProcessesByPath = (processes) => {
    ...
    for (const proc of processes) {
        let key = '';
        if (['V2'].includes(proc.data_sources[0].type)) {
            key = proc.data_sources[0].csv_detail.directory;
        } else {
            key = proc.data_sources[0].id;
        }
