...
class FunctionInfo {
    ...
    constructor(functionInfo = {}) {
        ...
        this.isMeFunction = isMeFunctionName(functionName);
        /** @type ?number */
    ...
    static collectInputFunctionInfo() {
        ...
        if (selectedVarXElement == null) {
            ...
            if (isMeFunctionName(functionName)) {
        ...
        const resultColumns =
            functionConfigElements.sampleDataElement.lastElementChild.querySelectorAll(
                'td:first-child',
            );
        const sampleDatas = [...resultColumns].map(
            (element) => element.dataset.originalValue ?? '',
        );
     ...
     */
    async fillInputFunctionInfo() {
        showResultFunction.disabled = true;
        functionConfigElements.functionNameElement.dataset.index =
            this.index ?? '';
        ...
        await select2TriggerChange(functionConfigElements.functionNameElement);
        functionConfigElements.varXElement.value = String(this.varX);
        await select2TriggerChange(functionConfigElements.varXElement);
        functionConfigElements.varYElement.value = String(this.varY);
        await select2TriggerChange(functionConfigElements.varYElement);
        ...
        showResultFunction.disabled = undefined;

        await handleShowResultData();
    }
    ...
    static resetInvalidStatus() {
        functionConfigElements.functionSettingModal
     ...
     */
    static async selectRowHandler(rowElement, previousRowElement = null) {
        removeAllSyncEvents();
        ...
        await functionInfo.fillInputFunctionInfo();
    ...
    static syncSampleDataToTable() {
        ...
        const configSampleDataNodes =
            functionConfigElements.sampleDataElement.querySelectorAll(
                'tbody>tr',
            );
        const configSampleData = Array.from(configSampleDataNodes).map(
            (node) => node.firstElementChild.dataset.originalValue ?? '',
        );
     ...
     * @private
     */
    static async #rowClickEvent(event) {
        ...
        if (selectedRowElement === rowElement) {
            ...
            if (inputFunctionInfo.isEqual(selectedFunctionInfo)) {
                ...
                await FunctionInfo.selectRowHandler(
                    rowElement,
                    selectedRowElement,
                );
                return;
            ...
            await FunctionInfo.selectRowHandler(rowElement);
        }
    }
     ...
     */
    static async rowClickEvent(event) {
        // lock function to do it at once in time
        if (FunctionInfo.rowClickEvent.disabled) return;

        try {
            FunctionInfo.rowClickEvent.disabled = true;
            await FunctionInfo.#rowClickEvent.call(this, event);
        } finally {
            // release lock
            FunctionInfo.rowClickEvent.disabled = undefined;
        }
    }
    ...
    addNewFunctionRow() {
        ...
        const newRowHtml =
            ...
            `<td class="column-sample-data row-item show-raw-text" data-column-title="sample_data">${this.sampleDatas[0] ?? ''}</td>` +
            `<td class="column-sample-data row-item show-raw-text" data-column-title="sample_data">${this.sampleDatas[1] ?? ''}</td>` +
            `<td class="column-sample-data row-item show-raw-text" data-column-title="sample_data">${this.sampleDatas[2] ?? ''}</td>` +
            `<td class="column-sample-data row-item show-raw-text" data-column-title="sample_data">${this.sampleDatas[3] ?? ''}</td>` +
            `<td class="column-sample-data row-item show-raw-text" data-column-title="sample_data">${this.sampleDatas[4] ?? ''}</td>` +
            '</tr>';
     ...
     */
    static async deleteFunctionRow(rowElement) {
        // Remove all events
        ...
        if (rowElement.classList.contains('selected')) {
            await FunctionInfo.resetStatusOfEditingFunction(rowElement);
        }
        ...
        for (const selectElement of [
            ...
            if (isTargetOption) {
                $(selectElement).val('');
                await select2TriggerChange(selectElement);
            }
        }
    ...
    static getAllFunctionInfosApi(processId, colIds) {
        ...
        for (const colId of colIds) {
            dictSampleData[colId] =
                ProcessConfigSection.collectSampleData(colId);
        }
        return fetchWithLog(
            '/ap/api/setting/function_config/get_function_infos',
            {
                method: 'POST',
                headers: {
     ...
     */
    static async loadFunctionListTableAndInitDropDown(functionData) {
        functionData
            .map((data) => new FunctionInfo(data))
            .forEach((functionInfo) => functionInfo.addNewFunctionRow());
        await FunctionInfo.initDropdownFunctionName();
    }

    /**
     * Remove all rows of Function table
     */
    static async removeAllFunctionRows() {
        for (const row of [
            ...functionConfigElements.functionTableElement.lastElementChild
                .children,
        ]) {
            await FunctionInfo.deleteFunctionRow(row);
        }
    }
     ...
     */
    static async resetInputFunctionInfo(
        ...
        showResultFunction.disabled = true;
        $(functionConfigElements.functionNameElement).val('');
        await select2TriggerChange(functionConfigElements.functionNameElement);
        $(functionConfigElements.varXElement).val('');
        await select2TriggerChange(functionConfigElements.varXElement);
        $(functionConfigElements.varYElement).val('');
        await select2TriggerChange(functionConfigElements.varYElement);
        showResultFunction.disabled = undefined;
     ...
     */
    static async resetStatusOfEditingFunction(selectedRow = null) {
        removeAllSyncEvents();
        await FunctionInfo.resetInputFunctionInfo(false, false);
        if (selectedRow == null) {
            selectedRow =
                functionConfigElements.functionTableElement.lastElementChild.querySelector(
                    'tr.selected',
     ...
     */
    static async initDropdownVarXY(selectedFunction) {
        const showSerial =
        ...
        const normalColumns = currentProcDataCols.filter((col) => {
            ...
            const isRepresentColumn = representColumnType.includes(
                col.column_type,
            );
            return !shouldSkipSerial && !isRepresentColumn;
        });
        ...
        const functionColumns = allFunctionInfos
            .filter(
                (col) =>
                    !col.isMeFunction &&
                    !ignoreFunctionColumns.includes(col.functionColumnId),
            )
            .map((col) => {
                return {
        ...
        [...normalColumns, ...functionColumns].forEach((col) => {
            const options = {
                ...
                'raw-data-type': col.raw_data_type,
        ...
        $varXElement.val(selectedValue['varXElement']);
        $varYElement.val(selectedValue['varYElement']);

        await select2TriggerChange($varXElement);
        await select2TriggerChange($varYElement);
    }

    /**
     * Initialize select2 for selection of Function Name
     */
    static async initDropdownFunctionName() {
        ...
        $functionNameElement.val('');
        await select2TriggerChange($functionNameElement);
    }
        ...
        switch (rawDataType) {
            ...
            case DataTypes.DATETIME.bs_value:
                label = document
                    ...
                    .textContent.trim();
                break;
            case DataTypes.CATEGORY.bs_value:
                label = document
    ...
    static validateInputFunctionInfo(inputFunctionInfo) {
        ...
        const isInvalidCoeANS =
            functionConfigElements.coeANSElement.classList.contains(
                BORDER_RED_CLASS,
            );
        const isInvalidCoeBK =
            functionConfigElements.coeBKElement.classList.contains(
                BORDER_RED_CLASS,
            );
        const isInvalidCoeCT =
            functionConfigElements.coeCTElement.classList.contains(
                BORDER_RED_CLASS,
            );
    ...
    static showErrorField(errorsResponse) {
        for (const error of errorsResponse) {
            if (ARRAY_ANS.includes(error.field)) {
                $(functionConfigElements.coeANSElement).addClass(
                    BORDER_RED_CLASS,
                );
            } else if (ARRAY_BK.includes(error.field)) {
                $(functionConfigElements.coeBKElement).addClass(
                    BORDER_RED_CLASS,
                );
            } else if (ARRAY_CT.includes(error.field)) {
                $(functionConfigElements.coeCTElement).addClass(
                    BORDER_RED_CLASS,
                );
            }
...
/**
 * Trigger `change` event for select2 control
 * @param {JQuery<HTMLSelectElement> | HTMLSelectElement} element
 * @return {Promise<any[]>}
 */
async function select2TriggerChange(element) {
    /** @type {JQuery<HTMLSelectElement>} */
    const $select = element instanceof jQuery ? element : $(element);
    const select = $select[0];
    const events = jQuery._data(select, 'events');

    // Access the specific event handler
    let outputHandlers = [];
    if (events.change && events.change.length > 0) {
        const event = { currentTarget: select };
        for (const change of events.change) {
            outputHandlers.push(await change.handler.call(select, event));
        }
    }
 ...
 */
async function addOrUpdateFunctionInfo(event) {
    const inputFunctionInfo = FunctionInfo.collectInputFunctionInfo();
 ...
 * @param {PointerEvent} event
 */
async function showRegisteredFunctionsModal(event) {
    // Expand input function info area
    $(functionConfigElements.collapseFunctionConfig).collapse('show');

    await FunctionInfo.resetStatusOfEditingFunction();
...
const copyFunctionHandle = async (el) => {
    await FunctionInfo.resetInputFunctionInfo(false, false);
 ...
 * @param {PointerEvent} event
 */
async function showFunctionDetailsModal(event) {
 ...
 * @param {PointerEvent} event
 */
async function registerFunctionInfo(event) {
    ...
    return ajaxWithLog({
        url: 'api/setting/function_config',
        type: 'POST',
        ...
        .done(async (res) => {
            hideAlertMessages();
            await FunctionInfo.removeAllFunctionRows();
            await FunctionInfo.resetStatusOfEditingFunction();
            await FunctionInfo.resetInputFunctionInfo();
            FunctionInfo.getAllFunctionInfosApi(procId, res.cfg_col_ids).then(
        ...
        .fail((res) => {
            ...
            if (errorResponses.code === 500) {
                ...
                for (const [functionColumnId, rowErrors] of Object.entries(
                    ...
                    row.querySelectorAll('.column-name-invalid').forEach(
                        (element) => element.classList.remove(BORDER_RED_CLASS),
                    );
        ...
        .always(async () => {
            await FunctionInfo.resetInputFunctionInfo(true, false);
            await FunctionInfo.resetStatusOfEditingFunction();
            FunctionInfo.updateStatusErrorCheckbox();
            await FunctionInfo.initDropdownFunctionName();
            loadingHide();
 ...
 * @param event
 */
async function pasteAllFunctionInfo(event) {
    const rawDataTypeTitle = {
        ...
        [DataTypes.SMALL_INT.name]: $(
            '#' + DataTypes.SMALL_INT.i18nLabelID,
        ).text(),
        ...
        for (const row of rows.slice(1)) {
            ...
            for (
                ...
                if (
                    ...
                    params[mapColumnNameDict[columnName]] =
                        fullWidthInputs.includes(columnName)
                            ? stringNormalization(row[columnIndex])
                            : row[columnIndex];
                }
            }
            ...
            } else if (isMeFunctionName(params.functionName)) {
                params.functionColumnId = -index;
 ...
 */
const handleDeleteFunctionColumns = async () => {
    for (const tr of [
        ...functionConfigElements.deleteFunctionColumnTableBody.children,
    ]) {
        const numberCell = tr.firstElementChild;
        const columnId = numberCell.dataset.functionColumnId;
        for (const columnFunctionName of functionConfigElements.functionTableElement.lastElementChild.querySelectorAll(
            `tr td[data-function-column-id='${columnId}']`,
        )) {
            const rowElement = columnFunctionName.parentElement;
            await FunctionInfo.deleteFunctionRow(rowElement);
...
const checkAnyChanges = (event) => {
    ...
    if (selectedRow != null) {
        ...
        const inputFunction = FunctionInfo.collectInputFunctionInfo();
        if (
            (inputFunction.functionId != null &&
                !isNaN(inputFunction.functionId)) ||
 ...
 */
const changeFunctionNameEvent = async (event) => {
    /** @type {HTMLSelectElement} */
    const el = event.currentTarget;
    ...
    let isMeFunction = isMeFunctionName(functionName);
    ...
    const changeStatusLabel = (attributeName, coeElement, coeLabelElement) => {
        ...
        if (coeLabel.length === 0) {
            ...
            coeElement.classList.remove(BORDER_RED_CLASS);
    ...
    const isAlreadyDisabled = showResultFunction.disabled;
    showResultFunction.disabled = true;
    await FunctionInfo.initDropdownVarXY(selectedOption);
...
const validateCoeInput = async (event) => {
    ...
    if (['a', 'b', 'c'].includes(coeLabel)) {
        ...
        if (!otherElementInvalid) {
            otherElement.classList.remove(BORDER_RED_CLASS);
        }
    }
    ...
    if (isValid) {
        ...
        await handleShowResultData(event);
    } else {
...
const changeFunctionColumnName = (el, colTableIdx) => {
    ...
    const isMeFunction = isMeFunctionName(selectedFunction.text);
    if (isMeFunction) {
        functionConfigElements.systemNameElement.value =
    ...
    if (Number(columnType) === masterDataGroup.GENERATED_EQUATION) {
        // collect sample data in function config table
        const row = $(functionConfigElements.functionTableElement)
            .find(`tr td[data-process-column-id=${columnId}]`)
            .parent()
            .last();
        sampleDataValues = [...row.find('td.column-sample-data')].map(
            (dataEl) => dataEl.textContent.trim(),
        );
    } else {
        ...
        if (rows.length) {
            ...
            let editingMeFunction = false;
            [...rows].every(function (row, index) {
                const functionName = $(row)
                    .find(`td.column-function-name label`)
                    .text();
                if (isMeFunctionName(functionName)) {
                    // not collect row selected(editing)
                    if ($(row).hasClass('selected')) {
                        editingMeFunction = true;
                        parentColumnIndex = index - 1; // parent is above row
                    }
                    if (index === 0 && editingMeFunction) {
                        // if edit first me function column, collect normal column
                        return false;
                    } else {
            ...
            if (childrenMeFunctionRows.length) {
                parentColumnIndex =
                    parentColumnIndex ?? childrenMeFunctionRows.length - 1;
                sampleDataValues = [
                    ...$(childrenMeFunctionRows[parentColumnIndex]).find(
                        'td.column-sample-data',
                    ),
                ].map((dataEl) => dataEl.textContent.trim());
            } else {
                sampleDataValues =
                    ProcessConfigSection.collectSampleData(columnId);
            }
        } else {
            sampleDataValues = ProcessConfigSection.collectSampleData(columnId);
        }
    }
...
const showResultFunction = delay(() => {
    ...
    return ajaxWithLog({
        url: '/ap/api/setting/function_config/sample_data',
        dataType: 'json',
        data: JSON.stringify(data),
        contentType: 'application/json',
        type: 'POST',
        success: (data) => {
            ...
            for (const index of Array(5).keys()) {
                ...
                elements[index].dataset.originalValue = val ?? '';
...
const selectFunction = async (el) => {
    const functionId = $(el).find('td[name="functionId"]').text();
    await FunctionInfo.resetInputFunctionInfo(false, false);
    $(functionConfigElements.functionNameElement).val(functionId);
    await select2TriggerChange(functionConfigElements.functionNameElement);
 ...
 * @param {HTMLAnchorElement} element
 */
const okFunctionConfirmSwitchEditingRowModal = async (element) => {
...
const syncInputConfigAndTableEvents = (configInput, tableInput, reload) => {
    async function syncTableToConfig() {
        configInput.value = stringNormalization(tableInput.value);
        if (reload) {
            await showResultFunction();
        }
    }

    async function syncConfigToTable() {
        tableInput.value = stringNormalization(configInput.value);
        if (reload) {
            await showResultFunction();
        }
    }
...
function collectFunctionDatasForRegister() {
    ...
    FunctionInfo.collectAllFunctionRows().forEach((functionInfo) => {
        ...
        const dicCfgProcessColumn = {
            ...
            bridge_column_name: functionInfo.systemName,
 ...
 * @param {PointerEvent} event
 */
async function reloadFunctionInfo(event) {
    loadingShowImmediately();
    await FunctionInfo.removeAllFunctionRows();
    await FunctionInfo.resetInputFunctionInfo();
    await FunctionInfo.resetStatusOfEditingFunction();
...
/**
 * Check if a function name is recognized as a me-function
 * This might be refactored in the future when we might use database column instead
 * @param {String} functionName
 * @return {Boolean}
 */
const isMeFunctionName = (functionName) => {
    return functionName.startsWith('me.');
};
...
(() => {
    ...
    $(functionConfigElements.functionNameElement).on('change', (e) =>
        changeFunctionColumnName(e.currentTarget, 1),
    );
    $(functionConfigElements.functionNameElement).on(
        'change',
        handleShowResultData,
    );
    $(functionConfigElements.varXElement).on('change', (e) =>
        changeFunctionColumnName(e.currentTarget, 2),
    );
    $(functionConfigElements.varXElement).on('change', handleShowResultData);
    $(functionConfigElements.varYElement).on('change', (e) =>
        changeFunctionColumnName(e.currentTarget, 3),
    );
