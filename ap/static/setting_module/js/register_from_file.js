...
const i18n = {
    ...
    dataType: document.getElementById('i18nDataType').textContent,
...
const registerSteps = {
    IMPORTING: 'importing',
    GEN_DATA_TABLE: 'GEN_DATA_TABLE',
    SCAN_FILE: 'SCAN_FILE',
    SCAN_MASTER: 'SCAN_MASTER',
    SCAN_DATA_TYPE: 'SCAN_DATA_TYPE',
    PULL_CSV_DATA: 'PULL_CSV_DATA',
    SCANNING: 'SCANNING',
};
...
async function handleResponseData(request) {
    ...
    const isOthers = data.datasourceConfig.master_type === 'OTHERS';
    renderProcessConfig(data.processConfigs, isOthers);
 ...
 */
function renderProcessConfig(data, isOthers) {
...
const saveDataSourceAndProc = async () => {
    ...
    // check input data source and process name
    $(registerFromFileEles.folderUrl).prop('disabled', true);
    $(registerFromFileEles.refFileUrl).prop('disabled', true);
