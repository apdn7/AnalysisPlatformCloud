const visualModule = (() => {
    ...
    const addConfigRow = (
        ...
        const controlColOptions = controlCols.map((col) => {
            const selected =
                Number(col.id) === Number(selectedControlCol)
                    ? eles.selectedOptStr
                    : '';
            if (Number(col.id) === Number(selectedControlCol)) {
                controlColName = col.shown_name;
            }
            return `<option value="${col.id}" title="${col.name_en}" ${selected}>${col.shown_name}</option>`;
        });

        const defaultFilterItemOption =
            '<option id="mcs-filterValue-io-result-no_${rowIdx}">---</option>';
        let contentOption = `<div class="custom-select2 d-flex">
                                        <span title="" style="width: 30%">Default</span>
                                        <span title="" style="width: 30%">デフォルト</span>
                                        <span title="" style="width: 40%">Sample</span>
                                    </div>`;
        const fromFilterConfigItemOptions = [
            defaultFilterItemOption.format(contentOption),
        ];
        cfgProcess.getFilters().forEach((filter) => {
            ...
            if (!isEmpty(filter.column_id)) {
                ...
                const filterItemName =
                    ...
                    filter.column_id ||
                    filter.name;
                filter.filter_details.sort((a, b) => {
                ...
                filter.filter_details.every((e, i) => {
                    return i <= 2;
                });
        ...
        const showAllItemOptions = [
            defaultFilterItemOption.format(i18nNames.partNoDefaultName),
        ];
    ...
    const register = () => {
        ...
        fetchWithLog(
            `/ap/api/setting/proc_config/${cfgProcess.id}/visualizations`,
            {
                method: 'POST',
                headers: {
    ...
    const sendSpreadSheetDataToSetting = (tableId, settingData, editData) => {
        ...
        for (const configRow of mergedConfigRows) {
            const [, rowDOM] = addConfigRow(configRow, false, true, index++);
            tblConfigDOM += rowDOM;
        }
        $(eles.tblConfigBody).html(tblConfigDOM);
    };
