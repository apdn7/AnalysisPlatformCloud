...
class ProcessConfigSection {
     ...
     * @param {ProcessConfig} processConfig - a process config object
     * @param {{}[]} sampleDataRows - a list of sample data
     * @param isOthers - is OTHERS data source
     * @return ProcessConfigSection - a process config section object
     */
    static createProcessConfigSectionForMain(
        processConfig,
        sampleDataRows,
        isOthers,
    ) {
            ...
            <table
               ...
               <thead>
                  <tr>
                     ...
                     <th class="format">${i18n.format}</th>
                  </tr>
               </thead>
               <tbody></tbody>
            </table>
...
`;
    ...
    static #generateOneRowOfSampleDataHTML(
        ...
        const tdHTMLs = [];
        const $checkboxDatetimeFormat = $('#toggleProcDatetimeFormat');
    ...
    static generateOneRowOfProcessColumnConfigHTML(
        ...
        const isRegisteredMainDatetimeColumn =
            col.is_get_date && isRegisteredProcess;
        return `
<tr class="${!col.is_show ? 'd-none' : ''} ${isDummyDatetime === true ? 'dummyDatetimeCol' : ''}"
    is-master-col="${col.is_master_col === true}"
>
    ...
    <td class="column-system-name" title="name_en">
        <input type="text" 
               ...
               value="${col.name_en ?? ''}" 
               ${checkedAttr} 
               ${isFixedName || col.is_master_col || isRegisteredMainDatetimeColumn ? 'disabled' : ''}
        >
    </td>
    ...
    <td class="column-format" title="format">
        <input type="text" 
               name="${procModalElements.format}" 
               class="form-control row-item" 
               value="${col.format || ''}" 
               ${!col.is_master_col && DataTypeDropdown_Controller.isDataTypeAllowFormat(col.raw_data_type) ? '' : 'disabled'}
               ${col.format}
               ${checkedAttr}
        >
    </td>
...
`;
    ...
    static handleMainDateAndMainTime(
        ...
        if (
            ...
            // In case there is a main::Date column or main::Time column and another column is selected as
            // main::Datetime -> change main::Date or main::Time column to normal type
            try {
                this.handleMainDateAndMainTime.disable = true; // lock to avoid running this function
                const mainColumn =
                    $spanMainTime.length > 0
                        ? $spanMainTime[0]
                        : $spanMainDate[0];
                DataTypeDropdown_Controller.changeToNormalDataType(mainColumn);
            } finally {
                this.handleMainDateAndMainTime.disable = undefined; // release lock
            }
        } else if (
            ...
            // In case there are main::Date and main::Time columns
            if (!isMainDateTimeDummyColumnExist) {
                $(procModalElements.msgContent).text(
