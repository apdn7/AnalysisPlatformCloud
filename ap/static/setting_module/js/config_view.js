const csvResourceElements = {
    ...
    apiImportToDBUrl: 'grpc_server/setting/import_csv_to_db',
    ...
    folderV2UrlInput: 'input[name="folderV2Url"]',
    folderV2HistoryUrlInput: 'input[name="folderV2HistoryUrl"]',
    isFilePathHidden: '#isFilePath',
    fileName: '#fileName',
    connectResourceBtn: '#connectResourceBtn',
    connectV2ResourceBtn: '#connectV2ResourceBtn',
    connectV2HistoryResourceBtn: '#connectV2HistoryResourceBtn',
    csvDiv: '#csvDiv',
    v2SourceDiv: '#v2SourceDiv',
    v2HistoryDiv: '#v2HistoryDiv',
    ...
    folderV2UrlId: '#folderV2Url',
    folderV2HistoryUrlId: '#folderV2HistoryUrl',
    directory: 'directory',
    ...
    csvDirectImportBtn: 'button.directImportBtn[data-csv="1"]',
    masterDbType: '#master_db_type',
    partitionModal: '#partitionModal',
    directColumnAttributeModal: '#directColumnAttributeModal',
...
const getAllDatabaseConfig = async () => {
    const json = await fetchWithLog('/ap/api/setting/database_tables', {
        method: 'GET',
        headers: {
...
const UpdatePollingFreq = (pf, withImport = 'NO') => {
    ...
    fetchWithLog('api/setting/update_polling_freq', {
        method: 'POST',
        headers: {
            Accept: 'application/json',
            'Content-Type': 'application/json',
...
const warningI18n = {
    ...
    mappingConfig: $('#i18nMappingConfig').text(),
 ...
 */
const showNewFoundMasterToastr = (dataTableName, dataTableId) => {
    const { mappingConfig } = warningI18n;
    const msgContent = `<p>
    "${dataTableName} ${$('#i18nHasNewMasterWhenScanningMsg').text()}"
    <br>
        <a style="float:right;" href="/ap/mapping_config/master_line?data_table_id=${dataTableId}" target="_blank">
            ${mappingConfig}
        </a>
    </p>`;
