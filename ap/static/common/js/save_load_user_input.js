...
const controlInvalidFilterLifeCycle = (key) => {
    const invalidFilterKeys =
        jsonParse(localStorage.getItem(INVALID_FILTER_DETAIL_IDS)) || [];
...
const saveInvalidFilters = (key, invalidFilterDetailIds) => {
    ...
    const ids = jsonParse(localStorage.getItem(key)) || [];
    targets.concat(ids);
...
const getSortKeys2 = (targetEle, isUserMoved = false) => {
    const $ele = $(targetEle);
    const dataTypeOrders = [
        DataTypes.DATETIME.short,
        DataTypes.SERIAL.short,
        DataTypes.JUDGE.short,
        DataTypes.MASTER.short,
        DataTypes.CATEGORY.short,
        DataTypes.REAL.short,
        DataTypes.INTEGER.short,
        DataTypes.STRING.short,
        DataTypes.BOOLEAN.short,
    ];
    const defaultOrderNo = 9999;
    const $dataTypeCol = $ele.find('.data-type');
    const $facetSelect = $ele.find('select');
    const $checkboxes = $ele.find('input:checkbox');
    const $radioBox = $ele.find('input:radio');
    const isColumnSelected = !!(
        $checkboxes.length > 0 && $checkboxes[0].checked
    );
    const isLabelSelected = !!(
        $checkboxes.length > 1 && $checkboxes[1].checked
    );
    const currentDataType = $dataTypeCol.text().trim();

    const facetOrderNo =
        $facetSelect.length > 0 && $facetSelect.val() !== ''
            ? Number($facetSelect.val())
            : defaultOrderNo;
    const columnSelectedOrderNo =
        isColumnSelected === null
            ? defaultOrderNo
            : isColumnSelected
              ? 0
              : defaultOrderNo;
    const radioSelectedOrderNo =
        $radioBox.length > 0
            ? $radioBox[0].checked
                ? 0
                : defaultOrderNo
            : defaultOrderNo;
    const labelSelectedOrderNo =
        isLabelSelected === null
            ? defaultOrderNo
            : isLabelSelected
              ? 0
              : defaultOrderNo;
    const groupOrderNos = [
        facetOrderNo,
        columnSelectedOrderNo,
        radioSelectedOrderNo,
        labelSelectedOrderNo,
    ];
    const isColumnSet = groupOrderNos.includes(0) || groupOrderNos.includes(1);
    const selectedCTColumn =
        isColumnSet && currentDataType === 'CT' ? 0 : defaultOrderNo;
    const dataTypeOrderNo = dataTypeOrders.indexOf(currentDataType);
    const originOrderNo = $dataTypeCol.data('origin-order-no');

    const priorityOrders = [];
    if (isUserMoved) {
        const columnSetOrderNo = isColumnSet ? 0 : defaultOrderNo;
        const userMovedOrderNo = $ele.parent().children().index($ele);

        priorityOrders.push(columnSetOrderNo);
        priorityOrders.push(userMovedOrderNo);

        return priorityOrders;
    }

    priorityOrders.push(selectedCTColumn);
    priorityOrders.push(facetOrderNo);
    priorityOrders.push(radioSelectedOrderNo);
    priorityOrders.push(columnSelectedOrderNo);
    priorityOrders.push(labelSelectedOrderNo);
    priorityOrders.push(dataTypeOrderNo);
    priorityOrders.push(originOrderNo);
...
const sortHtmlElements = (parentEle, isSimple = null) => {
    let isChanged = false;
    const ul = $(parentEle);
    const items = ul.find('.list-group-item:not(.keep-header)').get();
    const isUserMoved =
        items.filter((e) => e.style.position === 'relative').length > 0;
    consoleLogDebug(`[sortHtmlElements] isUserMoved: ${isUserMoved}`);
    items.sort((a, b) => {
        // const aKeys = getSortKeys(a, isSimple);
        // const bKeys = getSortKeys(b, isSimple);

        const aKeys = getSortKeys2(a, isUserMoved);
        const bKeys = getSortKeys2(b, isUserMoved);
    ...
    const innerFunc = (
        ...
        if (isLoad) {
            ...
            if (isSaveToLocalStorage) {
                data = jsonParse(localStorage.getItem(key));
            } else {
                data = savedData;
            }
...
const deleteUserSetting = (e) => {
    ...
    fetchWithLog(`/ap/api/setting/user_setting/${userSettingId}`, {
        method: 'DELETE',
        headers: {
...
const useUserSetting = (
    ...
    fetchWithLog('/ap/api/setting/load_user_setting', {
        method: 'POST',
        headers: reqHeaders,
        body: JSON.stringify({
...
const useUserSettingOnLoad = () => {
    ...
    fetchWithLog(`/ap/api/setting/user_setting_page_top?page=${page}`, {
        method: 'GET',
        headers: reqHeaders,
    })
        .then((response) => response.json())
...
const applyUserSetting = (userSetting, onlyLoad = null, isLoadNew = true) => {
    ...
    if (typeof userSetting.settings === 'string') {
        userInputs = jsonParse(userSetting.settings || '{}');
    } else {
...
const loadAllUserSetting = () => {
    ...
    fetchWithLog('/ap/api/setting/user_settings', {
        method: 'GET',
        headers: reqHeaders,
    })
        .then((response) => response.json())
    ...
    if (window.location.pathname === settingCommonInfo.page) {
        ...
        const filterData = {
            dic_cat_filters: lastUsedFormData
                ? jsonParse(lastUsedFormData.get('dic_cat_filters'))
                : '',
            temp_cat_exp: lastUsedFormData
                ? jsonParse(lastUsedFormData.get('temp_cat_exp'))
                : '',
            temp_cat_procs: lastUsedFormData
                ? jsonParse(lastUsedFormData.get('temp_cat_procs'))
                : '',
        };
...
const isSaveColumnOrdering = () => {
    if (_.isEmpty(currentLoadSetting) | 0) return false;
...
const getFilterOnDemand = () => {
    if (_.isEmpty(currentLoadSetting)) return null;
    const settings = jsonParse(currentLoadSetting.settings);
