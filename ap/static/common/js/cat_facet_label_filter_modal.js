...
function fillDataToFilterModal(data = filterDataModal, callback) {
    ...
    // mouseover 2 seconds
    let timeoutId = null;
    $('.show-detail:not(.click-only)').hover(
        function () {
            if (!timeoutId) {
...
function renderFilterHtml(filterData) {
    ...
    return filterData.map((cat) => {
        ...
        formatPattern[column_id] = end_col_format;
        let divColId = '';
        let divPagingId = '';
        if (!isDisabled) {
            summaries[column_id] = {
...
const genCheckBox = (columnId, value) => {
    const format = formatPattern[columnId];
    ...
    return `<div class="p-1 list-group-item">
             <div class="custom-control custom-checkbox custom-control-inline mr-0">
                ...
                <label class="custom-control-label" for="${id}"> ${isNA | isEmpty ? COMMON_CONSTANT.NA : applySignificantDigit(value, undefined, format)} </label>
                </div>
            </div>`;
};
...
const initGlobalDict = (data = filterDataModal) => {
    ...
    for (const filterData of Object.values(data)) {
        ...
        for (const cat of filterData) {
            const columnId = cat.column_id.toString();
...
const updateGlobalDict = (filterData) => {
    ...
    for (const colId of Object.keys(dicCheckboxes)) {
        if (!availableKeys.includes(colId)) {
