...
const buildSummaryResultsHTML = (
    ...
    const format = generalInfo.endProcFmt ?? '';
    if (beforeRankValues) {
    ...
    <table class="hist-summary-detail result basic-statistics" style="margin-top: 10px;">
        <tbody>
            ...
            <!-- In case of number padding, make it look like text column and no show calculated results -->
            ${
                !zeroFillPattern.test(format)
                    ? `
            <tr>
                <td><span class="item-name">${i18nCommon.average}</span></td>
                <td id="arrayNum-${tableIndex}">
                    ${isEmpty(summaryOption.bsAverage) || percentagePattern.test(format) ? '-' : applySignificantDigit(convertStringToNumber(summaryOption.bsAverage), undefined, format)}
                </td>
            </tr>
            <tr>
                <td><span class="item-name">3σ</span></td>
                <td id="stdev-${tableIndex}">
                    ${isEmpty(summaryOption.sigma3) || percentagePattern.test(format) ? '-' : applySignificantDigit(convertStringToNumber(summaryOption.sigma3), undefined, format)}
                </td>
            </tr>
            <tr>
                <td><span class="item-name">Cp</span></td>
                <td id="ucl-${tableIndex}">
                    ${isEmpty(summaryOption.cp) || percentagePattern.test(format) ? '-' : applySignificantDigit(convertStringToNumber(summaryOption.cp), undefined, format)}
                </td>
            </tr>
            <tr>
                <td><span class="item-name">Cpk</span></td>
                <td id="ucl-${tableIndex}">
                    ${isEmpty(summaryOption.cpk) || percentagePattern.test(format) ? '-' : applySignificantDigit(convertStringToNumber(summaryOption.cpk), undefined, format)}
                </td>
            </tr>
            <tr>
                <td><span class="item-name">σ</span></td>
                <td id="average-${tableIndex}">
                    ${isEmpty(summaryOption.sigma) || percentagePattern.test(format) ? '-' : applySignificantDigit(convertStringToNumber(summaryOption.sigma), undefined, format)}
                </td>
            </tr>
            <tr>
                <td><span class="item-name">${i18nCommon.maxValue || '最大値'}</span></td>
                <td id="ucl-${tableIndex}">
                    ${isEmpty(summaryOption.maxValue) ? '-' : applySignificantDigit(convertStringToNumber(summaryOption.maxValue), undefined, format)}
                </td>
            </tr>
            <tr>
                <td><span class="item-name">${i18nCommon.minValue || '最小値'}</span></td>
                <td id="ucl-${tableIndex}">
                    ${isEmpty(summaryOption.minValue) ? '-' : applySignificantDigit(convertStringToNumber(summaryOption.minValue), undefined, format)}
                </td>
            </tr>
            `
                    : ''
    ...
    <table class="hist-summary-detail result non-parametric" style="margin-top: 10px;">
        <tbody>
            <!-- In case of number padding & percentage, make it look like text column and no show calculated results -->
            ${
                zeroFillPattern.test(format) || percentagePattern.test(format)
                    ? ''
                    : `
            <tr>
                <td><span class="item-name hint-text" title="${i18nCommon.hoverMedian}">${i18nCommon.median || '中央値'}</span></td>
                <td id="stdev-${tableIndex}">
                    ${isEmpty(summaryOption.median) ? '-' : applySignificantDigit(convertStringToNumber(summaryOption.median), undefined, format)}
                </td>
            </tr>
            <tr>
                <td><span class="item-name hint-text" title="${i18nCommon.hoverP95}">P95</span></td>
                <td id="arrayNum-${tableIndex}">
                    ${isEmpty(summaryOption.p95) ? '-' : applySignificantDigit(convertStringToNumber(summaryOption.p95), undefined, format)}
                </td>
            </tr>
            <tr>
                <td><span class="item-name hint-text" title="${i18nCommon.hoverP75Q3}">P75 Q3</span></td>
                <td id="average-${tableIndex}">
                    ${isEmpty(summaryOption.p75Q3) ? '-' : applySignificantDigit(convertStringToNumber(summaryOption.p75Q3), undefined, format)}
                </td>
            </tr>
            <tr>
                <td><span class="item-name hint-text" title="${i18nCommon.hoverP25Q1}">P25 Q1</span></td>
                <td id="ucl-${tableIndex}">
                    ${isEmpty(summaryOption.p25Q1) ? '-' : applySignificantDigit(convertStringToNumber(summaryOption.p25Q1), undefined, format)}
                </td>
            </tr>
            <tr>
                <td><span class="item-name hint-text" title="${i18nCommon.hoverP5}">P5</span></td>
                <td id="ucl-${tableIndex}">
                    ${isEmpty(summaryOption.p5) ? '-' : applySignificantDigit(convertStringToNumber(summaryOption.p5), undefined, format)}
                </td>
            </tr>
            <tr>
                <td><span class="item-name hint-text" title="${i18nCommon.hoverIQR}">IQR</span></td>
                <td>
                    ${isEmpty(summaryOption.iqr) ? '-' : applySignificantDigit(convertStringToNumber(summaryOption.iqr), undefined, format)}
                </td>
            </tr>
            <tr>
                <td><span class="item-name hint-text" title="${i18nCommon.hoverNIQR}">NIQR</span></td>
                <td>
                    ${isEmpty(summaryOption.niqr) ? '-' : applySignificantDigit(convertStringToNumber(summaryOption.niqr), undefined, format)}
                </td>
            </tr>
            <tr>
                <td><span class="item-name hint-text" title="${i18nCommon.hoverMode}">Mode</span></td>
                <td>
                    ${isEmpty(summaryOption.mode) ? '-' : applySignificantDigit(convertStringToNumber(summaryOption.mode), undefined, format)}
                </td>
            </tr>
            `
            }
