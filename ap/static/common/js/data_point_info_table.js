...
const genTRItems = (firstCol, secondCol, thirdCol = null, is_gray = false) => {
    let trEle = '<tr>';
    ...
    // }
    trEle += `<td colspan="1" ${is_gray ? 'class="gray"' : ''}>${secondCol}</td>`;
    trEle += `<td colspan="1">${thirdCol || ''}</td>`;
    trEle += '</tr>';
    return trEle;
};
...
const genSimpleDataTable = (yValue, nTotal, masterInfo) => {
    const valueLabel = $('#i18nValue').text();
    let tblContent = '<tr>';
    tblContent += genTRItems(valueLabel, yValue);
    tblContent += genTRItems('N', nTotal);
    tblContent += genTRItems('', ''); // blank
    for (const key in masterInfo) {
        const [value, is_reserved_null] = masterInfo[key];
        tblContent += genTRItems(key, value, undefined, is_reserved_null);
    }
    tblContent += '</tr>';
    return tblContent;
};

const genHoverDataTable = (data, masterInfoDiv = {}, masterInfoColor = {}) => {
    let tblContent = '<tr>';
    for (const d of data) {
        const key = d[0];
        const value = d[1];
    ...
    tblContent += genTRItems('', '');
    for (const key in masterInfoDiv) {
        const [value, is_reserved_null] = masterInfoDiv[key];
        tblContent += genTRItems(key, value, undefined, is_reserved_null);
    }

    tblContent += genTRItems('', '');
    for (const key in masterInfoColor) {
        const [value, is_reserved_null] = masterInfoColor[key];
        tblContent += genTRItems(key, value, undefined, is_reserved_null);
    }
    tblContent += '</tr>';
    return tblContent;
...
const showInforTbl = (data, horizontal = true, chartID) => {
    if (data.points) {
        ...
        let yValue = horizontal
            ...
            : dataPoint.data.x[dpIndex];
        let masterId = null;
        if (dataPoint.data.rankcolstomasterids) {
            masterId = dataPoint.data.rankcolstomasterids[yValue] ?? null;
        }
        const format = dataPoint.data.xhoverformat ?? '';

        yValue = applySignificantDigit(yValue, undefined, format);
        ...
        const masterName = dataPoint.data.columnname ?? '';
        const masterInfo = getMasterInfoHovering(masterId, masterName);
        const dataTable = genSimpleDataTable(yValue, countValue, masterInfo);
...
const showSCPDataTable = (data, offset, chartID, type = dpInfoCons.scatter) => {
    const genDataTable = () => {
        ...
        for (const masters of [data.x_master, data.y_master]) {
