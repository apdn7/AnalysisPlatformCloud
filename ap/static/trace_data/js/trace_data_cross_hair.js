...
const scatterHorizontalline = (
    ...
    label: {
        ...
        position: 'center',
        content: format
            ? applySignificantDigit(yValue, undefined, format)
            : yValue,
...
const tsHorizonalLine = (yValue, color = CONST.CH_OTHER, format = '') => ({
    type: 'line',
    id: 'crosshair-y',
    mode: 'horizontal',
    scaleID: 'y',
...
const drawCrossHairOnDoubleClick = (clickPosition, selectedCanvasId) => {
    ...
    allChartCanvas.each(function f() {
        ...
        const end_col_format =
            graphStore.getArrayPlotData(canvasId).end_col_format;
        ...
        if (chartType === 'scatter') {
            ...
            if (scatterDataPoint) {
                ...
                if (!isEmpty(cXValue) && !isEmpty(cYValue)) {
                    ...
                    scatterChartObject.options.plugins.annotation.annotations[
                        'crosshair-y'
                    ] = scatterHorizontalline(cYValue, color, format);
            ...
            if (coTimeSeriesGraph) {
                ...
                if (!isEmpty(coYValue) && histChartObject) {
                    ...
                    histChartObject.options.plugins.annotation.annotations[
                        'crosshair-y'
                    ] = histHorizontalLine(coYValue, props, undefined, format);
        ...
        if (chartType === 'timeSeries') {
            ...
            if (!isEmpty(coClickedData)) {
                tsChartObject.options.plugins.annotation.annotations[
                    'crosshair-y'
                ] = tsHorizonalLine(coClickedData, color, format);
            }
...
const drawCrosshairSingleClick = (
    ...
    const end_col_format =
        graphStore.getArrayPlotData(selectedCanvasId).end_col_format;
    const format = end_col_format ? end_col_format : '';
    ...
    sameRowCanvases.each(function f() {
        ...
        if (chartType === 'scatter') {
            ...
            if (scatterChartObject) {
                scatterChartObject.options.plugins.annotation.annotations[
                    'crosshair-y'
                ] = scatterHorizontalline(yValue, color, format);
                scatterChartObject.update((mode = 'none'));
            }
        }
        ...
        if (chartType === 'histogram') {
            ...
            if (histChartObject) {
                ...
                histChartObject.options.plugins.annotation.annotations[
                    'crosshair-y'
                ] = scatterHorizontalline(yValue, props, color, format);
                histChartObject.update((mode = 'none'));
            }
        }
        ...
        if (chartType === 'timeSeries') {
            ...
            tsChartObject.options.plugins.annotation.annotations[
                'crosshair-y'
            ] = tsHorizonalLine(yValue, color, format);
            tsChartObject.update((mode = 'none'));
        }
    });
