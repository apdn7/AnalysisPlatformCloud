const StepBarChart = ($, paramObj) => {
    ...
    const genDataTable = (catName, masterInfo, nTotal, ratio) => {
    ...
    const externalTooltipHandler = (context) => {
        ...
        const [cateName, masterId, columnName, count, ratio] =
            getStepChartHoverInfo(dataIndex, categoryName, plotData);
        const masterInfo = getMasterInfoHovering(masterId, columnName);
        genDataPointHoverTable(
            genDataTable(cateName, masterInfo, count, ratio),
...
const getStepChartHoverInfo = (categoryIndex, categoryName, plotDat) => {
    ...
    if (!categoryName && plotDat.before_rank_values) {
        ...
        if (plotDat.is_master_col) {
            masterId = plotDat.rank_cols_to_master_ids[itemID];
            columnName = plotDat.end_col_name;
        }
    }
    ...
    return [categoryName, masterId, columnName, count, ratio];
};
