...
const buildTimeSeriesCardHTML = (chartOption, cssName) => {
    ...
    const { endProcFmt } = chartOption;
    const { facetFormat } = chartOption;
    const { endProcId } = chartOption;
...
const buildGraphContainerHTML = (chartOption) => {
    ...
    const { isCTCol } = chartOption;
    const { facetFormat } = chartOption;
    const { endProcId, sensorId } = chartOption;
    ...
    if (checkTrue(catExpBox)) {
        ...
        catExpBoxHTML = `<span class="show-detail cat-exp-box" title="${hasLevel2 ? 'Level1 | Level2' : 'Level1'}">${catExpBoxLabel}</span>`;
    }
...
const traceDataChart = (data, clearOnFlyFilter) => {
    ...
    for (let i = 0; i < data.array_plotdata.length; i++) {
        ...
        if (catExpBox === null || catExpBox === '') {
            catExpBox = COMMON_CONSTANT.NA;
        }
        if (typeof catExpBox === 'object') {
            catExpBox = catExpBox.map((val) =>
                val === null || val === '' ? COMMON_CONSTANT.NA : val,
            );
        ...
        const end_col_format = data.array_plotdata[i].end_col_format;
        const endProcFmt = end_col_format ? end_col_format : '';
        ...
        const chartOption = {
            ...
            isCTCol,
            endProcFmt,
            facetFormat: data.COMMON.catExpBoxFormat,
            unit: plotData.unit,
        };
...
const drawHistogramsTab = (
    ...
    for (let rowIdx = 0; rowIdx < data.row; rowIdx++) {
        ...
        for (let i = 0; i < numChart; i++) {
            ...
            const sensorType =
                procConfigs[end_proc_id].dicColumns[end_col_id].data_type;
            const fmt = procConfigs[end_proc_id].dicColumns[end_col_id].format;
            // add int format padding in allGroupNames
            const allGroupNames =
                isCategory ||
                sensorType === DataTypes.TEXT.name ||
                (sensorType === DataTypes.INTEGER.name &&
                    zeroFillPatternRaw.test(fmt))
                    ? getAllGroupOfSensor(
            ...
            <div class="his-content ${hisCardBorder}">
                 ${chartTitle}
                <div id="${canvasId}" class="hd-plot" plotdata-index="${i}"
            ...
            const end_col_format = data.array_plotdata[i].end_col_format;
            const format = end_col_format ? end_col_format : '';
            const columnName = data.array_plotdata[i].end_col_name;
            const columnId = data.array_plotdata[i].end_col_id;
            const rankColsToMasterIds =
                data.rank_cols_to_master_ids[columnId] ?? [];
            ...
            const histParam = {
                ...
                endColFmt: format,
                rankColsToMasterIds: rankColsToMasterIds,
