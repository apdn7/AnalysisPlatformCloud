...
function YasuTsChart(
    ...
    const externalTooltipHandler = (context) => {
        ...
        const getEndColumnFormat = () => {
            const currentTraceData = graphStore.getTraceData();
            const plotDataIndex = $(`#${canvasId}`).attr('plotdata-index') || 0;
            const end_col_format =
                currentTraceData.array_plotdata[plotDataIndex].end_col_format;
            return end_col_format ? end_col_format : '';
        };
        ...
        const getDatYVal = (dataPoint, dicRankLabels = null, outlierDict) => {
            ...
            const format = getEndColumnFormat();
            ...
            if (datasetIndex === CONST.IRREGULAR_DATASET) {
                ...
                if (isOutlierValue(dataIndex, outlierDict)) {
                    ...
                    return applySignificantDigit(
                        outlierDict[dataIndex],
                        undefined,
                        format,
                    );
                }
            ...
            return `${applySignificantDigit(y, undefined, format)}`;
        };
        ...
        const getDatYMinMaxVal = (dataPoint, dataMin, dataMax, outlierDict) => {
            ...
            return [
                minVal,
                applySignificantDigit(medVal, undefined, format),
                maxVal,
            ];
        };
        ...
        const genDataTable = (dataPoint, outlierDict, currentThreshold) => {
            ...
            // get master info
            const masterInfo = graphStore.getMasterInfo(canvasId, dataPoint);
            if (masterInfo) {
                ...
                y: {
                    ...
                    ticks: {
                        ...
                        callback: function (value) {
                            ...
                            // String Ranked label
                            const end_col_format =
                                graphStore.getArrayPlotData(
                                    canvasId,
                                ).end_col_format;
                            const format = end_col_format
                                ? end_col_format
                                : y_fmt;
...
const buildColumnHTML = (
    ...
    for (const idx in serialCols) {
        ...
        if (selectedCol) {
            if (Number(col.id) === Number(selectedCol)) {
    ...
    $(`select[name=${processName}]`).each(function changeProc() {
        ...
        if (!binded) {
            $(this).on('change', async function changeSerialColumn() {
                ...
                const selectedSerials = getSelectedOrderCols(
                    tableId,
                    serialName,
                );
