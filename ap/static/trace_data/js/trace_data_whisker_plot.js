const whiskerPlot = (ctx, prop, format) => {
    const left = 0;
    const right = 3;
    ...
    // update hover data
    updateHoverData(ctx.canvas.id, prop, format);
...
const drawSensorWhisker = (
    ...
    const format = graphStore.getArrayPlotData(sensorIdx).end_col_format;
    ...
    const chart = whiskerPlot(ctx, prop, format);
...
const updateHoverData = (canvasId, prop, format) => {
    const isPercentagePattern = percentagePattern.test(format);
            ...
            <table id="${canvasId}ParamTbl">
                <tbody>
                    <tr>
                      ...
                      <td>${percentagePattern.test(format) ? '-' : applySignificantDigit(prop.upperRange)}</td>
                    </tr>
                    <tr>
                      <td>Upper whisker</td>
                      <td>${prop.p100}</td>
            ...
            <table id="${canvasId}NonParamTbl">
                ...
                <tbody>
                    <tr>
                      ...
                      <td>${isPercentagePattern ? '-' : applySignificantDigit(prop.t_avg_p_3sigma)}</td>
                      <td>${isPercentagePattern ? '-' : applySignificantDigit(prop.avg_p_3sigma)}</td>
                    </tr>
                    <tr>
                      <td>Average +σ</td>
                      <td>${isPercentagePattern ? '-' : applySignificantDigit(prop.t_avg_p_sigma)}</td>
                      <td>${isPercentagePattern ? '-' : applySignificantDigit(prop.avg_p_sigma)}</td>
                    </tr>
                    <tr>
                      <td>Average</td>
                      <td>${isPercentagePattern ? '-' : applySignificantDigit(prop.t_average)}</td>
                      <td>${isPercentagePattern ? '-' : applySignificantDigit(prop.average)}</td>
                    </tr>
                    <tr>
                      <td>Average -σ</td>
                      <td>${isPercentagePattern ? '-' : applySignificantDigit(prop.t_avg_m_sigma)}</td>
                      <td>${isPercentagePattern ? '-' : applySignificantDigit(prop.avg_m_sigma)}</td>
                    </tr>
                    <tr>
                      <td>Average -3σ</td>
                      <td>${isPercentagePattern ? '-' : applySignificantDigit(prop.t_avg_m_3sigma)}</td>
                      <td>${isPercentagePattern ? '-' : applySignificantDigit(prop.avg_m_3sigma)}</td>
                    </tr>
                </tbody>
            </table>
        </div>
