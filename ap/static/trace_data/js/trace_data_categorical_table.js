...
const buildCategoryBoxes = (
    ...
    let isIndex = false;
    const cateCol = dicCate.data;
    const format = dicCate.end_col_format;
    const originDatas = dicCate.origin_data || [];
    const columnName = dicCate.column_name;
    if (timeCol === null) {
        isIndex = true;
    ...
    for (let i = 0; i < len; i++) {
        ...
        let rankId = '';
        let masterId = '';
        if (originDatas && dicCate.is_master_col) {
            rankId = originDatas[i];
            const rankCol = rankColToMasterIds[dicCate.column_id];
            masterId = rankCol[rankId];
        }
        ...
        if (currentCateVal !== prevCateVal) {
            ...
            boxes.push({
                cateName: applySignificantDigit(
                    convertStringToNumber(currentCateVal),
                    undefined,
                    format,
                ),
                columnName: columnName,
...
const createCateTableHTML = (
    ...
    for (const idx in cateBoxes || []) {
        ...
        if (box.count >= 0) {
            ...
            if (box.masterId) {
                hoverStr += `<br>`;
                const masterName = box.columnName;
                const masterInfo = getMasterInfoHovering(
                    box.masterId,
                    masterName,
                );
                for (const key in masterInfo) {
                    const [value, is_reserved_null] = masterInfo[key];
                    hoverStr += `${key}: ${is_reserved_null ? '<span class="gray">' + value + '</span>' : value}<br>`;
                }
            }
...
const produceCategoricalTable = (traceData, options = {}) => {
    ...
    const rankColToMasterIds = traceData.rank_cols_to_master_ids;
    category.forEach((dicCate) => {
        let boxes = null;
        if (!dicCate.isOverUniqueLimit) {
            if (xAxisOption === 'INDEX') {
                boxes = buildCategoryBoxes(dicCate, rankColToMasterIds);
            } else {
