...
INIT_PREVIEW_DATA_FOLDER = ROOT_PATH / 'tests' / 'e2e' / 'preview'
DATA_FOLDER = ROOT_PATH / 'data'
PREVIEW_DATA_FOLDER = DATA_FOLDER / 'preview'
CURRENT_DATABASE_PORT = 5432
DATABASE_PORT = 5550
CURRENT_DATABASE_HOST = 'localhost'
DATABASE_HOST = '192.168.1.58'
...
def change_host_for_database() -> None:
    with INIT_BASIC_CONFIG_FILE.open('r+') as f:
        text = f.read()
        text = text.replace(f'- host: {CURRENT_DATABASE_HOST}', f'- host: {DATABASE_HOST}')
        f.seek(0)
        f.write(text)
    print(f"Database host ({DATABASE_HOST}) is updated in '{INIT_BASIC_CONFIG_FILE}' file.")


def change_port_for_database() -> None:
    with INIT_BASIC_CONFIG_FILE.open('r+') as f:
        text = f.read()
        text = text.replace(f'- port: {CURRENT_DATABASE_PORT}', f'- port: {DATABASE_PORT}')
        f.seek(0)
        f.write(text)
    print(f"Database port ({DATABASE_PORT}) is updated in '{INIT_BASIC_CONFIG_FILE}' file.")


def replace_basic_config_file() -> None:
    shutil.copy(INIT_BASIC_CONFIG_FILE, BASIC_CONFIG_FILE)
    print(f"Copied '{INIT_BASIC_CONFIG_FILE}' folder to '{BASIC_CONFIG_FILE}' folder.")


def copy_preview_data() -> None:
    if os.path.isdir(DATA_FOLDER):
        shutil.rmtree(DATA_FOLDER)
        print(f"'{DATA_FOLDER}' folder is removed.")

    os.makedirs(DATA_FOLDER)
    shutil.copytree(INIT_PREVIEW_DATA_FOLDER, PREVIEW_DATA_FOLDER)
    print(f"Copied '{INIT_PREVIEW_DATA_FOLDER}' folder to '{PREVIEW_DATA_FOLDER}' folder.")


def setup(branch_name: str | None) -> None:
    if branch_name:
        change_host_for_database()
        change_port_for_database()
