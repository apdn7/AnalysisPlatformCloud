...
class CommonParam:
    ...
    hm_step: int
    hm_mode: int
    hm_function_real: str
    hm_function_cate: str
    hm_trim: int
...
class DicParam:
    ...
    def add_agp_color_vars(self):
        if self.common.agp_color_vars:
            color_vars = list(map(int, self.common.agp_color_vars.values()))
            # add col if not in current col_ids list
            already_col_ids = list(self.get_all_end_col_ids())
            color_vars = [col_var for col_var in color_vars if col_var not in already_col_ids]
