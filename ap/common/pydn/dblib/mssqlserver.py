...
class MSSQLServer:
    ...
    # という形で呼び出す
    @log_execution_time(prefix='MSSQL')
    @handle_read_only()
    ...
    def gen_sql_and_params(stmt: Select) -> tuple[str, dict[str, str]]:
        # TODO(khanhdq): use pymssql directly without call from sqlalchemy
        dialect = mssql.pymssql.dialect()
        dialect.paramstyle = 'pyformat'
        compiled_stmt = stmt.compile(dialect=dialect)
        return compiled_stmt.string, compiled_stmt.params

    @staticmethod
    def datetime_converter(column: sa.Column) -> sa.Column:
        """This is work around for sql server to avoid returning binary data"""
        try_convert = sa.func.try_convert(sa.sql.literal_column('DATETIME'), column, 1)
        try_cast = mssql.try_cast(column, mssql.DATETIME)
        return sa.func.coalesce(try_convert, try_cast)
