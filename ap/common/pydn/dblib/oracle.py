...
class Oracle:
    ...
    def insert_table_records(self, tblname, names, values, add_comma_to_value=True):
        ...
        logger.info('Dummy data was inserted to {0:s}!'.format(tblname))
    ...
    # という形で呼び出す
    @log_execution_time(prefix='ORACLE')
    @handle_read_only()
    ...
    def fetch_many(self, sql, size=10_000, params=None):
        ...
        logger.debug(sql)
        with self._create_cursor_with_date_time_format() as cur:
            sql = Oracle.convert_sql(sql)
            if params:
                logger.debug(params)
                cursor = cur.execute(sql, params)
    ...
    def _check_connection(self):
        if self.is_connected:
            return True
        # 接続していないなら
        logger.error('Connection is not Initialized. Please run connect() to connect to DB')
        return False
    ...
    def gen_sql_and_params(stmt: Select) -> tuple[str, dict[str, str]]:
        compiled_stmt = stmt.compile(dialect=oracle.dialect())
