...
def calc_summary_elements(plot):
    ...
    summaries = []
    df[ARRAY_Y] = df[ARRAY_Y].replace({pd.NA: np.nan})
    ...
    for chart_info in chart_infos:
        ...
        # Bridge datetime column is datetime64 with UTC
        act_from = chart_info.get(ACT_FROM)
        act_from_formatted = pd.to_datetime(act_from)
        if pd.isna(act_from_formatted):
            act_from_formatted = pd.to_datetime(pd.Timestamp.min, utc=True)

        act_to = chart_info.get(ACT_TO)
        act_to_formatted = pd.to_datetime(act_to)
        if pd.isna(act_to_formatted):
            # 9999-01-01 is out of bounds nanosecond timestamp
            # change to max Timestamp limitation
            ...
            act_to_formatted = pd.to_datetime(pd.Timestamp.max, utc=True)

        if (not act_from and not act_to) or (
            act_from_formatted <= convert_time(min_x, return_string=False)
            and convert_time(max_x, return_string=False) <= act_to_formatted
        ):
            df_threshold = df
        else:
...
def convert_to_float64_without_infinity(df: DataFrame, columns: list[str]):
    """
    Convert data type of columns in dataFrame to float64 with no any infinity value

    :param df: DataFrame: dataFrame
    :param columns: list[str]: list of column name want to convert data type
    :return: void - input dataFrame will be changed that columns were convert to float64 data type with
     non-infinity value
    """
    for col in columns:
        if col not in df:
        ...
        try:
            ...
            logger.exception(f'Cannot convert [{col}] column to float 64: {e}')
