...
def zip_a_file(zip_file, target_file_paths, target_file_names=None, backup_db=None):
    ...
    if backup_db:
        backup_file_name = f"backup_database_{datetime.now().strftime('%Y%m%d%H%M%S')}.zip"
        dirname = get_export_setting_path()
        if not os.path.exists(dirname):
            os.makedirs(dirname)
        path_file = get_zip_setting_full_path(backup_file_name)
        with open(path_file, 'wb') as f:  # use `wb` mode
...
def extract_setting_zip_file(zip_file):
    input_zip = ZipFile(zip_file)

    return {get_table_name_in_file(name): input_zip.read(name) for name in input_zip.namelist()}


def get_table_name_in_file(file_name):
...
def get_zip_setting_full_path(filename):
    if not filename:
        return None

    return os.path.join(get_export_setting_path(), get_basename(filename))


def get_zip_sample_default_config_path(filename):
    if not filename:
        return None

    return os.path.join(get_init_path(), get_basename(filename))
