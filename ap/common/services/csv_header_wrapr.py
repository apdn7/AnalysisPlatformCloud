...
from ap.common.services.normalization import normalize_list, unicode_normalize
...
def get_data_type(wrapr):
    ...
    for data_type in col_types:
        ...
        elif data_type in ['int', 'int32', 'integer']:
...
def summarize_header_as_df(hdr: dict, info: dict):
    ...
    head['main'] = [unicode_normalize(text) for text in head['main']]
    head['long'] = [unicode_normalize(text) for text in head['long']]
...
def add_suffix_if_duplicated(names):
    ...
    if len(duplicated) == 0:
        return names, is_dupl_cols, pd_names

    _max_item = len(names) + 1

    # TODO: currently, Bridge station cannot work fine with this suffix
    # Which cause V2 multi and V2 multi history cannot imported
    # Will fix this in another backlog
    # [a, a_02, a_03]
    suffix_format = (f'_{str(x - 1).zfill(2)!s}' if x > 1 else '' for x in range(1, 1000))
    # [a, a.1, a.2] for pandas
    pd_suffix_format = (f'.{str(x - 1)!s}' if x > 1 else '' for x in range(1, 1000))
    dic_suffix = dict(zip(duplicated, tee(suffix_format, len(duplicated))))
    pd_dic_suffix = dict(zip(duplicated, tee(pd_suffix_format, len(duplicated))))
    for idx, s in enumerate(names):
        try:
            suffix = str(next(dic_suffix[s]))
            pd_suffix = str(next(pd_dic_suffix[s]))
        except KeyError:
            continue
        else:
            names[idx] += suffix
            pd_names[idx] += pd_suffix
            if suffix:
