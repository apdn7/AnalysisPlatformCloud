...
def parse_multi_filter_into_one(dic_form):
    ...
    # add catExpBox, div format
    cat_exp_box = []
    cat_exp_box_fmt = []
    for i in range(1, 10):
        cat_exp = dic_parsed[COMMON].get(f'{CAT_EXP_BOX}{i}')
        if cat_exp:
            cat_exp_box.append(int(cat_exp))
            cat_exp_box_fmt.append(CfgProcessColumn.get_fmt_by_id(int(cat_exp)))

    dic_parsed[COMMON][CAT_EXP_BOX] = cat_exp_box
    dic_parsed[COMMON][CAT_EXP_BOX_FORMAT] = cat_exp_box_fmt

    if DIV_BY_CAT in dic_parsed[COMMON]:
        dic_parsed[COMMON][DIV_BY_CAT_FORMAT] = CfgProcessColumn.get_fmt_by_id(dic_parsed[COMMON][DIV_BY_CAT])
...
def bind_dic_param_to_class(dic_proc_cfgs: Dict[int, CfgProcess], trace_graph: TraceGraph, dic_card_orders, dic_param):
    ...
    # category expand
    cat_exps = get_cat_exps_from_dic_common(dic_common)
...
def get_cat_exps_from_dic_common(dic_common: Dict[Any, Any]) -> List[int]:
    cat_exps = []
