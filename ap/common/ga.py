...
# FIXME: This isn't a correct way to check.
#  this function should not exist in `GA` because it is unrelated to GA.
#  consider moving this elsewhere.
def is_app_source_dn() -> bool:
    """We enable features based on DN version"""
    return ga_info.app_source == AppSource.DN


# FIXME: This isn't a correct way to check.
#  this function should not exist in `GA` because it is unrelated to GA.
#  consider moving this elsewhere.
def is_running_in_window() -> bool:
    """Check if we are running on window to enable csv/tsv?"""
    return ga_info.app_os.system == OsSystem.WINDOWS
