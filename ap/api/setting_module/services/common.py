...
@grpc_api()
def save_user_settings(records, __FORCE_OFFLINE_MODE__=False, synced=False):
    inserted_records = []
    for record in records:
        cfg_user_setting = parse_user_setting(record)
        cfg_user_setting.synced = synced
        with make_session() as meta_session:
            data = insert_or_update_config(meta_session, cfg_user_setting)
        inserted_records.append(data.as_dict())

    return inserted_records


def sync_user_setting_to_bs():
    user_settings = CfgUserSetting.get_not_synced() or []
    user_settings = [user_setting.as_dict() for user_setting in user_settings]
    save_user_settings(user_settings, __FORCE_OFFLINE_MODE__=False, synced=True)
