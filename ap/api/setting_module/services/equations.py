...
# if TYPE_CHECKING:
from ap.setting_module.models import CfgProcessColumn, CfgProcessFunctionColumn, MData
...
def remove_all_function_columns(session, proc_id):
    ...
    if ids:
        ...
        MData.remove_by_col_ids(ids, session)
...
def validate_functions_calculation(process_id: int, validation_errors: FunctionErrors, functions: list[dict[str, Any]]):
    ...
    raw_data_types = {
        cfg_col.id: cfg_col.raw_data_type for cfg_col in CfgProcessColumn.get_by_process_id(str(process_id))
    }
...
def validate_functions_duplicated_names(
    ...
    cfg_process_columns = [cfg_col for cfg_col in cfg_process_columns if not cfg_col.is_function_column]
