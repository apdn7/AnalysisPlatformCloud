...
from ap.common.services.sse import MessageAnnouncer, background_announcer
...
def show_heatmap_data(
    ...
    return gen_heatmap_data(graph_param, dic_param, df)
...
def get_function_i18n(hm_function):
    ...
    return _('CHM' + hm_function.replace('_', HALF_WIDTH_SPACE).title().replace(HALF_WIDTH_SPACE, ''))
...
def graph_heatmap_data_one_proc(df, proc_cfg: CfgProcess, graph_param: DicParam, dic_col_func, var_agg_cols, agg_cols):
    ...
    if df is None or df.empty:
        ...
        for end_col in end_cols:
            ...
            hm_function = dic_col_func[proc_id].get(end_col.id)
            if hm_function:
