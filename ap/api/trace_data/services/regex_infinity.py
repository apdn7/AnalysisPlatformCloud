...
from ap.common.constants import CYCLE_ID, ID, ROWID, TIME_COL
...
EXCLUDE_COLS = [ID, CYCLE_ID, 'col_group_id', TIME_COL, ROWID]
...
def get_changed_value_after_validate(df_before: DataFrame, df_after: DataFrame):
    ...
    for col in df_before.columns:
        if not check_validate_target_column(col) or is_datetime64_any_dtype(df_before[col]):
            continue
...
@log_execution_time()
def validate_data_with_regex(df: DataFrame):
    if df.empty:
        return df
    ...
    for col in df.columns:
        ...
        if col in int_cols or col in float_cols or col in time_cols:
            continue
