...
def show_graph_pca(
    ...
    raise RuntimeError("currently, we don't use this")
    # return gen_trace_data(dic_proc_cfgs, graph_param, orig_graph_param)
...
def get_data_point_info(sample_no, df: DataFrame, graph_param: DicParam, dic_proc_cfgs, dic_serials, dic_get_dates):
    ...
    for proc in graph_param.array_formval:
        ...
        for col_id, col_name, show_name in zip(proc.col_ids, proc.col_names, proc.col_show_names):
            ...
            if col_label in dic_serials.values():
                order = f'{proc_cnt:03}_1'
            elif col_label in dic_get_dates.values():
            ...
            col_format = CfgProcessColumn.get_fmt_by_id(col_id)
            col_infos.append((proc_cfg.shown_name, show_name, row[col_label], order, col_format))
