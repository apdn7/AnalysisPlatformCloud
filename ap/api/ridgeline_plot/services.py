...
def show_trace_data_by_categorical_var(
    ...
    dic_param, df = gen_trace_data_by_categorical_var(graph_param, dic_param, RLP_MAX_GRAPH)
    if not is_return_df:
        df = None
...
def show_trace_data_by_cyclic(
    ...
    dic_param, df = gen_trace_data_by_cyclic(graph_param, dic_param, RLP_MAX_GRAPH)
    if not is_return_df:
        df = None
...
def show_trace_data_by_term(
    ...
    dic_param, df = gen_rlp_data_by_term(graph_param, dic_param, RLP_MAX_GRAPH)
    if not is_return_df:
        df = None
...
def gen_trace_data_by_cyclic(graph_param, dic_param, max_graph=None):
    ...
    if CAT_ON_DEMAND not in dic_param:
        # Add this property to do not occur error on JS
...
def calc_emd_for_ridgeline(data, group_id, num_bins, signed=True, diff=False):
    ...
    for sensor in np.arange(num_sensors):
        ...
        # if there is no data , quit ( avoid error )
        if not len(x_wo_none):
            continue
            ...
            if len(array_y):
                ...
                if group_name in dic_group_by_cat:
                    ...
                    plotdata = gen_blank_rlp_plot(
                        ...
                        sensor_id=sensor,
                        sensor_name=dic_plot[END_COL_NAME],
                        end_col_format=dic_plot[END_COL_FORMAT],
                        group_name=group_name,
                    )
...
def gen_custom_plotdata(dic_proc_cfgs, dic_data, sensors, cat_div_id, max_graph):
    ...
    for sensor_id in sensors:
        ...
        fmt = convert_to_d3_format(cfg_col.format)
...
def gen_blank_rlp_plot(proc_name='', proc_id='', sensor_name='', sensor_id='', end_col_format='', group_name=''):
    return {
        RL_DATA: [],
        RL_GROUPS: [],
        RL_RIDGELINES: [],
...
def add_threshold_configs(dic_param, graph_param):
    try:
        chart_infos_by_cond_procs, chart_infos_org = get_chart_infos(graph_param)
