...
from ap.api.common.services.show_graph_database import ShowGraphConfigData
...
def gen_stats_table(
    ...
    for proc_order, proc_id in enumerate(proc_ids):
        ...
        col_eng_names = end_proc.eng_names
        ...
        if not pd.isna(time_val) and len(str(time_val)) > 0:  # Bridge datetime column has datatype datetime not string
            # dt_obj = parser.parse(time_val)
            dt_obj = time_val.astimezone(client_timezone)
        ...
        for _col_idx, _col_id in enumerate(col_ids):
            ...
            # Item
            eng_name = col_eng_names[col_idx]
            row.append(eng_name)
...
def convert_and_format(time_val, client_timezone, out_format=DATE_FORMAT_STR_CSV):
    with contextlib.suppress(Exception):
        dt_obj = pd.Timestamp(time_val).astimezone(client_timezone)
...
def gen_list_table(graph_param, df, client_timezone):
    ...
    for proc in graph_param.array_formval:
        ...
        cfg_process: ShowGraphConfigData = graph_param.dic_proc_cfgs[proc_id]
        get_date_col: CfgProcessColumn = cfg_process.get_date_col(column_name_only=False)
        line_col_cfgs: list[CfgProcessColumn] = cfg_process.get_lines(column_name_only=False)
        equip_col_cfgs: list[CfgProcessColumn] = cfg_process.get_equips(column_name_only=False)
        part_col_cfgs: list[CfgProcessColumn] = cfg_process.get_parts(column_name_only=False)
        serial_col_cfgs = cfg_process.get_serials(column_name_only=False)
        ...
        for serial in serial_col_cfgs:
            serial_label = gen_sql_label(serial.id, serial.column_name)
            if serial_label not in df.columns.tolist() or df.empty:
                continue
            master_ids.append(serial.id)
        ...
        # Line No
        for line in line_col_cfgs:
            master_ids.append(line.id)
            row = get_data_proc_row(dic_id_col, line.id, df, time_val, client_timezone)
        ...
        # Machine No
        for equip in equip_col_cfgs:
            master_ids.append(equip.id)
            row = get_data_proc_row(dic_id_col, equip.id, df, time_val, client_timezone)
            proc_rows.append(row)

        # Part No
        for part in part_col_cfgs:
            master_ids.append(part.id)
            row = get_data_proc_row(dic_id_col, part.id, df, time_val, client_timezone)
            proc_rows.append(row)
        ...
        for col_id in col_ids:
            ...
            if col_id in master_ids:
                continue

            if col_id == get_date_col.id:
                continue
            row = get_data_proc_row(dic_id_col, col_id, df, time_val, client_timezone)
...
def get_data_proc_row(dic_id_col, col_id, df, time_val, client_timezone):
    row = []
    cfg_col: CfgProcessColumn = dic_id_col.get(col_id)
    # Item
    row.append(cfg_col.name_en)
    ...
    if paths:
        ...
        for proc in graph_param.array_formval:
            ...
            col_ids = [
                col.id
                for col in columns
                if (col.column_type in DataGroupType.get_column_type_show_graph() or len(col.function_details))
            ]
