...
from ap.common.common_utils import convert_to_d3_format, gen_sql_label, zero_variance
...
def show_graph_sankey_group_lasso(
    graph_param,
    dic_param,
    df=None,
    ...
    return gen_graph_sankey_group_lasso(graph_param, dic_param, df)
    ...
    if not df.empty:
        ...
        if data_clean and not errors:
            ...
            for col in df_sensors.columns.to_list():
                ...
                if not col_cfg:
                    continue
                # cat sensors is data type Text or Int_Cate. Bridge station does not have int category
            ...
            dic_scp[OBJ_FORMAT] = convert_to_d3_format(obj_var_format)
