...
from ap.setting_module.models import CfgProcess, CfgProcessColumn, make_session
...
def show_trace_data_by_categorical_var(
    graph_param,
    dic_param,
    max_graph_per_tab,
    ...
    return gen_trace_data_by_categorical_var(graph_param, dic_param, max_graph_per_tab)


@grpc_api(show_graph=True)
def show_trace_data_by_cyclic(
    graph_param,
    dic_param,
    max_graph_per_tab,
    __FORCE_OFFLINE_MODE__=False,
    ...
    return gen_trace_data_by_cyclic(graph_param, dic_param, max_graph_per_tab)


@grpc_api(show_graph=True)
def show_trace_data_by_term(
    graph_param,
    dic_param,
    max_graph_per_tab,
    __FORCE_OFFLINE_MODE__=False,
    ...
    return gen_trace_data_by_term(graph_param, dic_param, max_graph_per_tab)
...
def gen_trace_data_by_categorical_var(graph_param, dic_param, max_graph=None, df=None):
    ...
    # add master mapping for rank values
...
def add_threshold_configs(dic_param, orig_graph_param):
    try:
        chart_infos_by_cond_procs, chart_infos_org = get_chart_infos(orig_graph_param)
...
def gen_trace_data_by_term(graph_param, dic_param, max_graph=None, df_cache=None):
    ...
    for term_id, term in enumerate(terms):
        ...
        term_result, df_term, graph_param = gen_graph_term(
            graph_param,
            term_dic_param,
            max_graph,
            df_cache,
...
def split_data_by_condition(dic_data, graph_param, max_graph=None):
    ...
    for proc in graph_param.array_formval:
        ...
        if not dic_data_for_df:
            continue

        dic_data_for_df.update(
            {TIME_COL: dic_data[proc_id].get(TIME_COL)},
        )
    ...
    for end_col, cat_exp_data in dic_data.items():
        ...
        for cat_exp_value, data in cat_exp_data.items():
            ...
            plotdata = {
                ...
                END_COL: end_col,
                # TODO(khanhdq): merge-code 208, edge server use shown_name here,
                # so we should use this as well, however this might lead to bugs with hovering
                END_COL_NAME: cfg_col.column_name,
                END_COL_FORMAT: convert_to_d3_format(cfg_col.format),
...
def use_etl_spray_shape(proc: CfgProcess):
    # TODO: after add Data Table , process belongs to n Data Source , so we can relate this process to Data Source.
    # TODO: check PO about below code
