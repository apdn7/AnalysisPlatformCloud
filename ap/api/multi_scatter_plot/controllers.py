...
def trace_data():
    ...
    dic_param, dic_data = show_scatter_plot(
        orig_graph_param,
        cache_dic_param or dic_param,
        df,
    ...
    if order_array_formval:
        ...
        for formval in order_array_formval:
            ...
            data = [val for val in dic_param[ARRAY_PLOTDATA] if str(val[END_COL_ID]) == str(col_id)]
            if len(data):
    ...
    if not dic_param[AS_HEATMAP_MATRIX]:
        dic_param[SCATTER_CONTOUR], is_show_contour_only = gen_scatter_n_contour_data(
            dic_param,
            dic_data,
            use_contour,
            orig_graph_param,
