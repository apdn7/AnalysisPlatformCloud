...
from ap.common.common_utils import convert_to_d3_format
...
@log_execution_time()
@abort_process_handler()
def gen_scatter_n_contour_data_pair(
    dic_param,
    array_y1,
    array_y2,
    use_contour,
    ...
    try:
        ...
        if not is_show_contour_only and not use_contour:
            ...
            scatter_data = {
                ...
                'x_user_fmt': array_y1_fmt,
                'y_user_fmt': array_y2_fmt,
                'mode': 'markers',
            }
...
@log_execution_time()
@abort_process_handler()
def gen_scatter_n_contour_data(dic_param: dict, dic_data, use_contour, graph_param: DicParam):
    scatter_contours = {}
    ...
    for i in range(num_sensor - 1):
        ...
        array_y_i = dic_data[int(proc_id_i)][int(col_id_i)]
        cfg_col_i = graph_param.get_col_cfg(col_id_i)
        array_y_i_format = convert_to_d3_format(cfg_col_i.format)
        ...
        for k in range(i + 1, num_sensor):
            ...
            array_y_k = dic_data[int(proc_id_k)][int(col_id_k)]
            cfg_col_k = graph_param.get_col_cfg(col_id_k)
            array_y_k_format = convert_to_d3_format(cfg_col_k.format)
...
def get_org_plotdat(plotdat):
    ...
    # bridge station does not have INT_CATEGORY
...
def calc_partial_corr(dic_param):
    ...
    standard_df = df.replace(dict.fromkeys(ALL_SYMBOLS, np.nan))
    clean_df = standard_df.dropna()
    p_corr_mat = None
    if len(clean_df):
    ...
    for k, col in enumerate(columns):
        ...
        for i, row in enumerate(columns):
            df_pair = standard_df[[col, row]].dropna()
            if df_pair.empty:
                continue
...
def clear_unused_data(dic_param):
    for plot_data in dic_param.get(ARRAY_PLOTDATA, []):
        for key in [ARRAY_X, ARRAY_Y, UNLINKED_IDXS, NONE_IDXS, INF_IDXS, NEG_INF_IDXS]:
            if key in plot_data:
                del plot_data[key]

    for key in [SERIALS, DATETIME, CYCLE_IDS]:
        if key in dic_param:
            del dic_param[key]
