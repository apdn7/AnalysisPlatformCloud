...
def add_backup_data_job(process_id, start_time, end_time):
    job_name = job_id = f'{JobType.BACKUP_DATABASE.name}_{process_id}'
    dic_params = {
        '_job_id': job_id,
        '_job_name': job_name,
...
def backup_data_job(_job_id, _job_name, *args, **kwargs):
    ...
    send_processing_info(gen, JobType.BACKUP_DATABASE, process_id=process_id)
    add_gen_proc_link_job(process_id=process_id, is_user_request=True)


def add_restore_data_job(process_id, start_time, end_time):
    job_name = job_id = f'{JobType.RESTORE_DATABASE.name}_{process_id}'
    dic_params = {
        '_job_id': job_id,
        '_job_name': job_name,
        'process_id': process_id,
...
def restore_data_job(_job_id, _job_name, *args, **kwargs):
    ...
    send_processing_info(gen, JobType.RESTORE_DATABASE, process_id=process_id)
    add_gen_proc_link_job(process_id=process_id, is_user_request=True)
