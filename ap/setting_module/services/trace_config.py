...
def get_all_processes_traces_info():
    ...
    dic = get_datasource_process_info()
    for proc in procs:
        proc.data_sources = dic.get(proc.id)
    return dump_json(procs)


def get_datasource_process_info():
    data_ids, data_table_ids = MappingProcessData.get_process_data_table_id()
    _data_ids, process_ids = MData.get_process_id_data_id()
    df1 = pd.DataFrame({'data_id': data_ids, 'data_table_id': data_table_ids})
    df2 = pd.DataFrame({'data_id': _data_ids, 'process_id': process_ids})
    df = df1.merge(df2, on=['data_id']).drop_duplicates(subset=['data_table_id', 'process_id'])
    cfg_data_tables = CfgDataTable.get_in_ids(ids=df.data_table_id.unique().tolist())
    data_table_dict = {x.id: x.data_source for x in cfg_data_tables}
    result_dict = {}
    for idx, row in df.iterrows():
        if row.process_id not in result_dict:
            result_dict[row.process_id] = []
        result_dict[row.process_id].append(data_table_dict.get(row.data_table_id))
...
def gen_cfg_trace(trace):
    ...
    self_process_id = int(trace.get('from'))
    target_process_id = int(trace.get('to'))
