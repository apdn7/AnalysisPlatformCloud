...
class DataTableColumnSchema(ma.SQLAlchemyAutoSchema):
    class Meta:
        model = CfgDataTableColumn
        exclude = EXCLUDE_COLS
        unknown = marshmallow.EXCLUDE

    data_group_type = fields.Integer(required=False, allow_none=True)

    @post_load
    def make_obj(self, data, **kwargs):
        return CfgDataTableColumn(**data)


class DataTableSchema(ma.SQLAlchemyAutoSchema):
    class Meta:
        model = CfgDataTable
        include_fk = True
    ...
    data_source_name = fields.String(required=False, allow_none=True)
    data_source = Nested(DataSourceSchema, required=False, allow_none=True)
    columns = Nested(DataTableColumnSchema, many=True)
    detail_master_type = fields.String(required=False, allow_none=True)
    mapping_page_enabled = fields.Boolean(required=False)

    @post_load
    def make_obj(self, data, **kwargs):
        return CfgDataTable(**data)
...
class TraceKeySchema(ma.SQLAlchemyAutoSchema):
    ...
    id = fields.Integer(required=False, allow_none=True)
    trace_id = fields.Integer(required=False, allow_none=True)
...
class TraceSchema(ma.SQLAlchemyAutoSchema):
    ...
    id = fields.Integer(required=False, allow_none=True)
    self_process_id = fields.Integer(required=False)
    target_process_id = fields.Integer(required=False)
    trace_keys = Nested(TraceKeySchema, many=True)


class ProcessLiteSchema(ma.SQLAlchemyAutoSchema):
    class Meta:
        model = CfgProcess
        include_fk = True
        include_relationships = True
        exclude = ('visualizations', 'traces', 'filters') + EXCLUDE_COLS  # re-open the params if used
...
class ProcessSchema(ma.SQLAlchemyAutoSchema):
    ...
    data_sources = Nested(DataSourceSchema, many=True, required=False, allow_none=True)
    data_tables = Nested(DataTableSchema, many=True, required=False, allow_none=True)
    data_source_name = fields.String(required=False, allow_none=True)
    data_table_name = fields.String(required=False, allow_none=True)
    name_en = fields.String(required=False, allow_none=False)
    name_jp = fields.String(required=False, allow_none=True)
    name_local = fields.String(required=False, allow_none=True)
    shown_name = fields.String(required=False, allow_none=True)
    is_imported = fields.Boolean(required=False, allow_none=False)
...
class ProcessFullSchema(ma.SQLAlchemyAutoSchema):
    ...
    data_tables = Nested(DataTableSchema, many=True)
