...
if exist %path_python% if exist %path_getpip% if exist %path_oracle% if exist %path_database% (
...
if exist %path_database% (echo Detect postgresql) else call :downloadPostgresql
...
IF exist %path_database_zip% del %path_database_zip%
echo:
echo Removed Oracle and Python embedded and Postgresql zipped files.
echo:
...
python.exe main.py
ECHO Analysis Platform Stopped !!!
call .\stop_db.bat
...
: _____________________________________________________________________________
: Subroutine: Download Postgres SQL Database
:      In/Out - |In -
:downloadPostgresql
  echo Download Postgresql
  powershell wget %postgresql_url% -O %path_database_zip%
  if %errorlevel% == 35 (
    REM In case CA cert in local machine is expired or disabled --- download CA cert to authenticate
    if exist %ca_cert% (echo Detect ca_cert) else call :downloadCACert
    powershell wget -Certificate %ca_cert% -Uri %postgresql_url% -O %path_database_zip%
  )
  if errorlevel 1 (
    echo %esc%[41m Error on Wget Check network connection or use latest Win10 ^>1803 %esc%[0m
    echo 999 > %stage_status%
  ...
  echo Unzip Postgresql
  powershell -Command "Expand-Archive -Path %path_database_zip% -DestinationPath %PGTARGET_PATH%"
  exit /b
:end
...
:defSetting
  ...
  set path_database=%cd%\..\database\pgsql
  set path_database_zip=%cd%\..\pgsql.zip
  ...
  set postgresql_url="https://sbp.enterprisedb.com/getfile.jsp?fileid=1258632"
...
setlocal
  ...
  echo   - file_mode: %file_mode% >> %filename%
  echo   - browser_debug: %browser_debug% >> %filename%
  echo - database: !!omap >> %filename%
  echo   - dbname: %PGDATABASE% >> %filename%
  echo   - host: %PGHOST% >> %filename%
  echo   - port: %PGPORT% >> %filename%
  echo   - username: %PGUSER% >> %filename%
  echo   - password: %PGPASSWORD% >> %filename%
endlocal
exit /b
:end
